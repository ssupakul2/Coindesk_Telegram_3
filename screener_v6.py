...
def log_partial_close(position: dict, coin: str, exit_price: float) -> None:
    """บันทึก Partial TP1 event ลง trade log (position ยังไม่ถูกลบจาก positions.json)"""
    append_trade_log({
        "coin": coin,
        "event": "partial_tp1",
...
def close_position(positions: dict, coin: str, reason: str, exit_price: float | None = None) -> None:
        ...
        if coin in positions:
            ...
            if status["action"] in ("close_sl", "close_tp2", "close_runner_sl", "time_stop"):
                ...
                if status["action"] in ("close_sl", "close_tp2", "close_runner_sl"):
                    close_position(positions, coin, status["action"], exit_price=price)
                ...
                log_partial_close(pos, coin, exit_price=price)
