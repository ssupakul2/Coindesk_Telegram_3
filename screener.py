...
def _fetch_4h_from_coingecko(coin: str) -> pd.DataFrame | None:
    """
    ดึง 4H OHLC จาก CoinGecko พร้อมแก้ปัญหา:
    1. Staleness detection — ตรวจอายุ candle ล่าสุด
    2. Real-time price patch — ถ้า candle เก่าเกิน DATA_STALENESS_WARN_HOURS
       จะดึง current price จาก /simple/price แล้วอัปเดต close/high/low ของ candle ล่าสุด
    """
    cg_id = COINGECKO_IDS.get(coin)
    if not cg_id:
