...
def calculate_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Fix 1: Strip leading zero-variance bars
      ถ้า coin เพิ่ง list หรือมี candles ราคาเดียวกันในช่วงแรก
      ก่อน RSI calculation จะ strip rows ที่ close = 0 หรือ ก่อน first price change
      เพื่อไม่ให้ EWM contaminated โดย flat period
    ...
    """
    # Strip leading zero-price rows (ก่อน listing หรือ frozen period)
    close_raw = df["close"]
    first_nonzero = (close_raw > 0).idxmax()
    if first_nonzero != close_raw.index[0]:
        df = df.loc[first_nonzero:].copy()

    # Strip leading constant-price rows (price stuck at one value for too long)
    # หา index แรกที่ราคาเริ่มขยับ
    if len(df) > 20:
        price_changes = df["close"].diff().abs()
        cumulative_changes = (price_changes > 0).cumsum()
        # ข้ามแท่งที่ price ไม่เคยขยับเลยตั้งแต่ต้น
        meaningful_start = cumulative_changes[cumulative_changes >= 3].index
        if len(meaningful_start) > 0 and meaningful_start[0] != df.index[0]:
            bars_stripped = df.index.get_loc(meaningful_start[0])
            if bars_stripped > 0 and bars_stripped < len(df) - 50:
                df = df.loc[meaningful_start[0]:].copy()
    ...
    # Fix 2: fillna(50) ไม่ใช่ fillna(100) — neutral เมื่อ no variance
    # ป้องกัน false overbought signal สำหรับ thinly-traded coins
    df["RSI"] = (100 - (100 / (1 + rs))).fillna(50)

    # RSI Sanity Clamp: ถ้า RSI > 90 แต่ price variance ต่ำมาก (degenerate data)
    # → clamp เป็น 50 (neutral) เพื่อป้องกัน false exit watch signal
    ...
    if last_rsi > 90:
        ...
        if cv_check < 0.05 or n_uniq <= 3:
            # RSI สูงเกิน 90 + variance ต่ำ = ข้อมูลไม่น่าเชื่อถือ → set neutral
            df["RSI"] = df["RSI"].clip(upper=75)
            logger.warning(
                f"RSI Clamp: RSI was {last_rsi:.1f} → clipped to ≤75 "
