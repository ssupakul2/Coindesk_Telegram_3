...
# RSI >= RSI_EXIT_HARD_OVERRIDE -> force เข้า exit watch ทันทีโดยไม่ต้องรอ score
RSI_EXIT_HARD_OVERRIDE = 78
# RSI >= RSI_EXIT_WARN_THRESHOLD -> ลด effective threshold ลงครึ่งนึง (25 แทน 40)
# เพราะ RSI overbought เดี่ยวๆ ก็น้ำหนักพอแล้ว
RSI_EXIT_WARN_THRESHOLD = 70
...
def calculate_exit_score(rsi, bearish_candle_info, vol_confirmed, mtf_info, adx, is_bear_div, trend_info, onchain_info, price, ema50, ema200, near_overbought_target: bool, df: pd.DataFrame | None = None) -> tuple[int, str, list]:
    """
    ...
    # ------------------------------------------------------------------
    # 2. RSI Velocity — RSI พุ่งขึ้นเร็วเกินไปใน 3-5 แท่งล่าสุด = exhaustion
    # ------------------------------------------------------------------
    if df is not None and "RSI" in df.columns and len(df) >= 6:
        try:
            rsi_series = df["RSI"].iloc[-6:]
            rsi_rise_3 = float(rsi_series.iloc[-1] - rsi_series.iloc[-4])  # rise in 3 bars
            rsi_rise_5 = float(rsi_series.iloc[-1] - rsi_series.iloc[-6])  # rise in 5 bars
            if rsi_rise_3 >= 15:
                score += 15
    ...
    # ------------------------------------------------------------------
    # 4. Bearish Divergence (price HH, RSI LH)
    # ------------------------------------------------------------------
    if is_bear_div:
        score += 20
    ...
    # Volume Climax: spike สูงมากแล้วราคาไม่ไปต่อ = distribution
    if df is not None and "volumeto" in df.columns and "VOL_MA20" in df.columns:
        try:
            ...
            if not pd.isna(vol_ma) and vol_ma > 0 and vol_now > vol_ma * 2.5:
                if rng > 0 and body / rng < 0.4:  # big volume + small body = absorption/distribution
                    score += 12
                    reasons.append("Volume Climax + Small Body (Distribution สัญญาณ)")
        except Exception:
            pass
...
def scan_market(positions: dict):
    ...
    for coin in COINS:
        ...
        if price > ema200:
            ...
            if weekly_ctx.get("fibo_786") and price <= weekly_ctx["fibo_786"] * 1.02:
                ...
                elif bounce_info["quality"] == "strong":
    ...
    summary_msg = (
        ...
        f"💼 Positions: {len(positions)}/{MAX_CONCURRENT_POSITIONS} | High-BTC-Corr: {existing_high_corr_count}/{MAX_HIGH_CORR_POSITIONS}\n\n"
...
def build_messages(buy_list, exit_watch_list, position_updates, market_summary, supply_zone_alerts=None) -> list:
    ...
    if exit_watch_list:
        ...
        for opt in sorted(exit_watch_list, key=lambda x: -x["exit_score"]):
            ...
            # RSI Hard Alert header — โดดเด่นกว่าปกติ
            rsi_header = ""
            if opt.get("rsi_hard_alert"):
