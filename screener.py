...
# หมายเหตุ: ใช้สำหรับดึง On-Chain และใช้เป็นท่อสำรองเมื่อตระกูล Binance โดน Geo-Block 451
CRYPTOCOMPARE_API_KEY = str(os.getenv("CRYPTOCOMPARE_API_KEY") or "").strip()

PORTFOLIO_USDT      = 1500.0
RISK_PER_TRADE_PCT  = 2.0
MAX_TOTAL_RISK_PCT  = 6.0   # จำกัดความเสี่ยงรวมต่อรอบ
...
# ==========================================
API_RATE_LIMIT_DELAY = 0.5   # หน่วงเวลาปลอดภัยสำหรับ API
API_MAX_RETRIES      = 3
BINANCE_LIMIT        = 500   # ดึงย้อนหลัง 500 แท่งเพื่อคำนวณ EMA200 ได้แม่นยำ

CACHE_TTL_SECONDS    = 3600  # ป้องกันสเปมสคริปต์ซ้ำซ้อนภายใน 1 ชั่วโมง

# --- Binance Cluster Gateways (อัปเดตสำหรับท่อ VPN) ---
BINANCE_ENDPOINTS = [
    "https://api.binance.com",  # โดเมนหลัก (VPN ทะลวงบล็อกให้แล้ว)
...
MINIMUM_SIGNAL_SCORE = 50   # แจ้งเตือนเฉพาะไม้ที่มีคะแนนความมั่นใจกึ่งหนึ่งขึ้นไป

# --- On-Chain Integration Target ---
...
# ==========================================
# Global API Session Setup (สำหรับ Binance)
# ==========================================
api_session = requests.Session()
api_session.headers.update({
    "User-Agent": "CryptoScreenerBot/3.6 (Binance Dynamic Multi-Source Engine)",
    "Accept":     "application/json"
})

retry_strategy = Retry(
    total=API_MAX_RETRIES,
    backoff_factor=2.0, # ปรับเพิ่มเพื่อให้เวลารอการเชื่อมต่อนานขึ้นเมื่อเจออาการหน่วงของ VPN
...
def send_telegram_messages(chunks: list) -> None:
    ...
    if not token or not chat_id:
        logger.error("TELEGRAM_BOT_TOKEN หรือ TELEGRAM_CHAT_ID ยังไม่ได้ตั้งค่าใน Environment Variables")
        return
    ...
    for idx, chunk in enumerate(chunks, start=1):
        ...
        try:
            ...
            logger.error(f"เกิดข้อผิดพลาดขณะส่ง Telegram (ส่วนที่ {idx}): {e}")
        if idx < len(chunks):
            time.sleep(0.5)

# ==========================================
# Data Fetching (Bypass Geo-block ด้วย Isolated Request & Debug Engine)
# ==========================================
def get_historical_data(coin: str) -> pd.DataFrame | None:
    ...
    # ทางเลือกที่ 1: วนลูปดึงตรงจาก Binance Cluster Gateways
    for base_url in BINANCE_ENDPOINTS:
        url = f"{base_url}/api/v3/klines"
        try:
            resp = api_session.get(url, params=params, timeout=15) # ขยาย Timeout เป็น 15 วินาที
            if resp.status_code == 200:
                data = resp.json()
                ...
                logger.warning(f"{coin} 4H ลองยิง {base_url} ติดรหัส: {resp.status_code} -> กำลังสลับเกตเวย์...")
        except Exception as e:
            logger.warning(f"{coin} 4H ผ่าน {base_url} เกิดข้อผิดพลาด Network: {e} -> กำลังลองตัวถัดไป...")
            
    # ทางเลือกที่ 2: (Bypass Engine) ใช้ Clean Session ยิงตรงหา Global Aggregator 
    logger.warning(f"⚠️ {coin} 4H ติดปัญหา Geo-Block/Network บน Binance กำลังสลับไปใช้ท่อกลางสำรองระดับโลก...")
    ...
    try:
        resp = requests.get(backup_url, params=backup_params, headers={"User-Agent": "Mozilla/5.0"}, timeout=15)
        if resp.status_code == 200:
            res_json = resp.json()
            
            if res_json.get("Response") == "Error":
                logger.error(f"❌ CryptoCompare ปฏิเสธ Request: {res_json.get('Message')}")
                return None
                
            if "Data" in res_json and "Data" in res_json["Data"]:
                raw_list = res_json["Data"]["Data"]
                if len(raw_list) > 0:
                    df = pd.DataFrame(raw_list)
                    
                    logger.info(f"🔍 [DEBUG 4H] คอลัมน์ที่ได้รับจากท่อสำรอง: {list(df.columns)}")
                    ...
                    if "volumeto" not in df.columns:
                        if "volumefrom" in df.columns:
                            df["volumeto"] = df["volumefrom"]
                        elif "volume" in df.columns:
                            df["volumeto"] = df["volume"]
                        else:
                            df["volumeto"] = 0.0

                    required_cols = ["open", "high", "low", "close", "volumeto"]
                    ...
                    logger.info(f"✅ {coin} 4H ดึงข้อมูลสำเร็จผ่านท่อกลางสำรอง (หลบเลี่ยง Geo-block 451 สำเร็จ)")
                    return df[required_cols]
            else:
                logger.error(f"❌ โครงสร้าง JSON ของ CryptoCompare ผิดเพี้ยน: ไม่พบคีย์ ['Data']['Data']")
        else:
            logger.error(f"❌ ท่อสำรองตอบกลับด้วย HTTP Status Code: {resp.status_code}")
            
    except Exception as e:
        logger.error(f"❌ ท่อกลางสำรอง 4H เกิดข้อผิดพลาดภายในระบบคำนวณ: {e}", exc_info=True)
        
    logger.error(f"❌ {coin} 4H ดึงข้อมูลไม่สำเร็จหลังจากลองครบทุกมาตรการแล้ว")
    return None

def get_histoday_data(coin: str) -> pd.DataFrame | None:
    """ดึงข้อมูลแท่งเทียน 1D พร้อมระบบหมุนเวียน Gateway และสลับ Source สำรองแบบป้องกัน Error โครงสร้างข้อมูลซ้อน"""
    symbol = f"{coin}USDT"
    params = {
        "symbol": symbol,
        "interval": "1d",
    ...
    for base_url in BINANCE_ENDPOINTS:
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                logger.warning(f"{coin} 1D ลองยิง {base_url} ติดรหัส: {resp.status_code} -> กำลังสลับเกตเวย์...")
        except Exception as e:
            logger.warning(f"{coin} 1D ผ่าน {base_url} เกิดข้อผิดพลาด Network: {e} -> กำลังลองตัวถัดไป...")
            
    # ทางเลือกที่ 2: (Bypass Engine) สลับไปใช้ท่อกลางสำรองระดับโลกเมื่อติดปัญหา Geo-block 451
    logger.warning(f"⚠️ {coin} 1D ติดปัญหา Geo-Block/Network บน Binance กำลังสลับไปใช้ท่อกลางสำรองระดับโลก...")
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if "Data" in res_json and "Data" in res_json["Data"]:
                ...
                if len(raw_list) > 0:
                    ...
                    logger.info(f"🔍 [DEBUG 1D] คอลัมน์ที่ได้รับจากท่อสำรอง: {list(df.columns)}")
                    ...
                    logger.info(f"✅ {coin} 1D ดึงข้อมูลสำเร็จผ่านท่อกลางสำรอง (หลบเลี่ยง Geo-block 451 สำเร็จ)")
                    return df[required_cols]
            else:
                logger.error(f"❌ โครงสร้าง JSON ของ CryptoCompare ผิดเพี้ยน: ไม่พบคีย์ ['Data']['Data']")
        else:
        ...
        logger.error(f"❌ ท่อกลางสำรอง 1D เกิดข้อผิดพลาดภายในระบบคำนวณ: {e}", exc_info=True)
        
    logger.error(f"❌ {coin} 1D ดึงข้อมูลไม่สำเร็จหลังจากลองครบทุกมาตรการแล้ว")
    return None

# ==========================================
# Bulk Processing & Cache Engines
# ==========================================
def bulk_fetch_4h(coins: list) -> dict:
    global _cache_4h, _cache_ts_4h
    now = time.time()
    if _cache_4h and (now - _cache_ts_4h) < CACHE_TTL_SECONDS:
        logger.info("ดึงข้อมูล 4H จากหน่วยความจำจำลอง (Cache Hit)")
        return _cache_4h
    result = {}
    for coin in coins:
    ...
    _cache_4h    = result
    _cache_ts_4h = now
    logger.info(f"Bulk Fetch ข้อมูล 4H ครบถ้วน: {len(result)}/{len(coins)} เหรียญ")
    return result
...
def bulk_fetch_1d(coins: list) -> dict:
    ...
    if _cache_1d and (now - _cache_ts_1d) < CACHE_TTL_SECONDS:
        logger.info("ดึงข้อมูล 1D จากหน่วยความจำจำลอง (Cache Hit)")
        return _cache_1d
    result = {}
    for coin in coins:
    ...
    _cache_1d    = result
    _cache_ts_1d = now
    logger.info(f"Bulk Fetch ข้อมูล 1D ครบถ้วน: {len(result)}/{len(coins)} เหรียญ")
    return result
...
# ==========================================
# Advanced Multi-Timeframe Scoring Engines
# ==========================================
...
def get_correlation_adjusted_position(
    portfolio: float, risk_pct: float, sl_distance_pct: float, corr_btc: float, active_signals_count: int
) -> float:
...
def get_mtf_rsi_alignment(df_4h: pd.DataFrame, df_1d: pd.DataFrame) -> dict:
    ...
    score = 0
    if rsi_4h <= RSI_OVERSOLD: score += 2
    elif rsi_4h <= 45: score += 1

    if rsi_1d <= RSI_OVERSOLD: score += 2
    elif rsi_1d <= 45: score += 1

    result["confluence_score"] = score

    if score >= 4:
        result["aligned_oversold"] = True
        result["mtf_label"] = f"💎 MTF RSI Oversold พร้อมกัน! (4H:{rsi_4h:.1f} / 1D:{rsi_1d:.1f})"
    elif score >= 2:
        result["mtf_label"] = f"🟡 MTF RSI อ่อนแรงปานกลาง (4H:{rsi_4h:.1f} / 1D:{rsi_1d:.1f})"
    elif rsi_4h >= RSI_OVERBOUGHT and rsi_1d >= RSI_OVERBOUGHT:
        result["aligned_overbought"] = True
        result["mtf_label"] = f"🔴 MTF RSI Overbought พร้อมกัน! (4H:{rsi_4h:.1f} / 1D:{rsi_1d:.1f})"

    return result

def calculate_signal_score(
    rsi: float, bounce_info: dict, candle_info: dict, vol_confirmed: bool,
    in_fibo_zone: bool, in_ob_zone: bool, in_fvg_zone: bool, weekly_ctx: dict,
    mtf_info: dict, adx: float, is_divergence: bool, trend_info: dict
) -> tuple[int, str]:
    score   = 0
    factors = []
    ...
    if rsi <= RSI_OVERSOLD:
        ...
        score += 15; factors.append(f"RSI Low({rsi:.1f})")
    elif rsi <= 50:
        score += 8
    ...
    score += bq
    if bq >= 12: factors.append("RSI Bounce")

    candle_pts = min(candle_info.get("reversal_strength", 0) * 7, 15)
    score += candle_pts
    if candle_pts > 0: factors.append("Candle Confirm")

    if in_fibo_zone: score += 8;  factors.append("Fibo Zone")
    if in_ob_zone: score += 7;  factors.append("OB Zone")
    if in_fvg_zone: score += 5;  factors.append("FVG Zone")

    if is_divergence: score += 10; factors.append("Bullish Div")
    if vol_confirmed: score += 10; factors.append("Volume OK")

    mtf_pts = min(mtf_info.get("confluence_score", 0) * 2, 10)
    score  += mtf_pts
    if mtf_info.get("aligned_oversold"): factors.append("MTF Aligned")
    ...
    if ts == "strong_up": score += 5;  factors.append("Strong Uptrend")
    elif ts == "moderate_up": score += 2

    score = max(0, min(100, score))
    if score >= 70: grade = "🔥 A+ (แนะนำเข้าซื้อ)"
    elif score >= 55: grade = "✅ A  (พิจารณาเข้าซื้อ)"
    elif score >= MINIMUM_SIGNAL_SCORE: grade = "🟡 B  (รอยืนยันสัญญาณ)"
    else: grade = "⬜ C  (สัญญาณอ่อนมาก)"

    label = f"{grade} | Score: {score}/100 | [{', '.join(factors)}]"
    return score, label

# ==========================================
# Timeframe Context Analyzers (1W / 1M)
# ==========================================
def analyze_weekly_context(df_1d: pd.DataFrame) -> dict:
    ...
    try:
        ...
        fibo_window = df_w.iloc[-52:] if len(df_w) >= 52 else df_w
        ...
        if result["weekly_bullish_div"]:
            result["weekly_status_label"] = f"👑 <b>เกิด Weekly Bullish Divergence!</b> (RSI: {curr_rsi_w})"
        elif curr_rsi_w <= RSI_OVERSOLD:
            result["weekly_status_label"] = f"🔥 <b>ภาพใหญ่ 1W Oversold รุนแรง ({curr_rsi_w})</b>"
        elif result["fibo_786"] and curr_price <= result["fibo_786"]:
            result["weekly_status_label"] = "⚠️ ราคาลงทดสอบโซน Deep Discount (ใต้ Fibo 78.6%)"
        elif curr_rsi_w >= RSI_OVERBOUGHT:
            result["weekly_status_label"] = f"⚠️ ภาพใหญ่ Overbought ({curr_rsi_w}) ระวังปรับฐาน"
        else:
            result["weekly_status_label"] = f"↔️ ภาพใหญ่ปกติ (RSI 1W: {curr_rsi_w})"
    except Exception as e:
...
def analyze_monthly_targets(df_1d: pd.DataFrame) -> dict:
    ...
    if df_1d is None or len(df_1d) < 30: return result
    try:
        df_m = df_1d.resample("ME").agg(
            {"open": "first", "high": "max", "low": "min", "close": "last"}
        ).dropna()
        if len(df_m) < 2: return result
        ...
        r1    = (2 * pivot) - m_low
        s1    = (2 * pivot) - m_high
        m_ema12 = (df_m["close"].ewm(span=12, adjust=False).mean().iloc[-1] if len(df_m) >= 12 else pivot)
        curr_price = current_month["close"]
        if curr_price >= m_ema12:
            result.update({
                "m_resistance_target":  max(r1, m_high), "m_support_target": pivot,
                "monthly_summary_label": "🔮 <b>ภาพ 1M (Bullish):</b> เป้าหมายราคาทดสอบกรอบบน", "monthly_trend": "bullish",
            })
        else:
            result.update({
                "m_resistance_target":  pivot, "m_support_target": min(s1, m_low),
                "monthly_summary_label": "🔮 <b>ภาพ 1M (Bearish):</b> แนวโน้มไหลลงหาแนวรับกรอบล่าง", "monthly_trend": "bearish",
            })
...
def analyze_cycle_targets(df_1d: pd.DataFrame) -> dict:
    ...
    if df_1d is None or len(df_1d) < 60: return result
    try:
        df_w = df_1d.resample("W").agg(
            {"open": "first", "high": "max", "low": "min", "close": "last"}
        ).dropna()
        if len(df_w) < 4: return result
        curr_price = df_w["close"].iloc[-1]
        macro_high, macro_low = df_w["high"].max(), df_w["low"].min()
        fib_1618 = macro_low  + ((macro_high - macro_low) * 1.618)
        fib_0618 = macro_low  + ((macro_high - macro_low) * 0.618)

        potential = []
        if macro_high > curr_price * 1.1: potential.append(("Historical Resistance", macro_high))
        if fib_0618 > curr_price * 1.1: potential.append(("Macro Fibo 61.8%", fib_0618))
        if fib_1618 > curr_price * 1.1: potential.append(("Fibo Extension 161.8%", fib_1618))
        ...
        if potential:
            ...
            result.update({
                ...
                "cycle_summary_label": f"🎯 <b>เป้าหมายรอบไซเคิล:</b> <code>${format_price(nt[1])}</code> <i>({nt[0]})</i>",
            })
    except Exception: pass
    return result
...
def check_death_cross_1d(df_1d: pd.DataFrame) -> dict:
    ...
    if df_1d is None or len(df_1d) < 200: return result
    try:
        df       = df_1d.copy()
        ...
        for i in range(1, len(df_last)):
            ...
            if p50 >= p200 and c50 < c200:
                ...
                result["death_cross_label"] = "☠️ <b>Warning! เกิด Death Cross (1D) ในช่วง 1 เดือนที่ผ่านมา</b>"
                break
    except Exception as e:
...
# ==========================================
# On-Chain Metric System
# ==========================================
def analyze_onchain_momentum(coin: str) -> dict:
    result = {
        "onchain_warning":      False, "onchain_label":        "🔹 On-Chain (1M): ปกติ",
        "large_tx_change_pct":  0.0,
    }
    if not CRYPTOCOMPARE_API_KEY:
        result["onchain_label"] = "🔹 On-Chain: ข้าม (ไม่มี API Key)"
        return result

    url = "https://min-api.cryptocompare.com/data/blockchain/histo/day"
    params = {"fsym": coin, "limit": 30, "api_key": CRYPTOCOMPARE_API_KEY}
    try:
        resp = api_session.get(url, params=params, timeout=10)
        data = resp.json()
        if data.get("Response") == "Success" and isinstance(data.get("Data", {}).get("Data"), list):
            df_oc = pd.DataFrame(data["Data"]["Data"])
            if len(df_oc) >= 15:
                tx_col = next((c for c in ["large_transaction_count", "transaction_count"] if c in df_oc.columns), None)
                if tx_col:
                    ...
                    if prior_avg > 0:
                        ...
                        if chg > 40.0:
                            ...
                            result["onchain_label"] = f"🚨 <b>On-Chain Warning:</b> วาฬโอนพุ่ง {chg:.1f}%"
                        else:
                            result["onchain_label"] = f"🔹 On-Chain (1M): ปกติ ({chg:+.1f}%)"
    except Exception:
        result["onchain_label"] = "🔹 On-Chain: ข้าม (API Key Limit)"
    return result
...
def analyze_futures_context(coin: str) -> dict:
    result = {
        "futures_warning": False, "futures_label": "📊 Futures (1M): ปกติ / ไม่มีข้อมูล",
    }
    symbol = f"{coin}USDT"
    try:
        fr_url = f"https://fapi.binance.com/fapi/v1/premiumIndex?symbol={symbol}"
        fr_resp = api_session.get(fr_url, timeout=10)
        
        oi_url    = f"https://fapi.binance.com/futures/data/openInterestHist?symbol={symbol}&period=1d&limit=30"
        oi_resp   = api_session.get(oi_url, timeout=10)
        
        if fr_resp.status_code == 200 and oi_resp.status_code == 200:
            ...
            if len(oi_data) >= 2:
                ...
                if oi_chg > 25.0 and fr_pct > 0.015:
                    result.update({"futures_warning": True, "futures_label": f"🚨 <b>Futures: ระวัง Long Squeeze!</b> OI +{oi_chg:.1f}%, FR {fr_pct:.4f}%"})
                elif oi_chg > 25.0 and fr_pct < -0.015:
                    result.update({"futures_warning": True, "futures_label": f"🚨 <b>Futures: ระวัง Short Squeeze!</b> OI +{oi_chg:.1f}%, FR {fr_pct:.4f}%"})
                else:
                    result["futures_label"] = f"📊 Futures: ปกติ (OI {oi_chg:+.1f}%, FR {fr_pct:.4f}%)"
    except Exception: pass
    return result

# ==========================================
# Technical Core Math Helpers
# ==========================================
...
def estimate_price_for_target_rsi(df: pd.DataFrame, target_rsi: float = 70.0, period: int = RSI_PERIOD) -> float | None:
    if len(df) < period + 1: return None
    try:
        close, delta = df["close"], df["close"].diff()
...
def find_fair_value_gaps(df: pd.DataFrame) -> dict:
    ...
    for i in range(len(df) - 1, 2, -1):
        ...
        if l_cur > h_m2 and c_m1 > o_m1:
            ...
            if gap_pct >= FVG_THRESHOLD_PCT and df["close"].iloc[-1] > h_m2:
...
def analyze_trend_continuity(df: pd.DataFrame) -> dict:
    ...
    if both_up and up_streak >= TREND_MIN_CONSECUTIVE: strength, label = "strong_up", f"🚀 ขาขึ้นแข็งแกร่ง ({up_streak} แท่ง)"
    elif slope50 > 0 and up_streak >= 1: strength, label = "moderate_up", f"📈 ขาขึ้นปานกลาง ({up_streak} แท่ง)"
    elif both_down and dn_streak >= TREND_MIN_CONSECUTIVE: strength, label = "strong_down", f"🔻 ขาลงแข็งแกร่ง ({dn_streak} แท่ง)"
    elif slope50 <= 0 and dn_streak >= 1: strength, label = "moderate_down", f"📉 ขาลงปานกลาง ({dn_streak} แท่ง)"
...
def analyze_rsi_bounce(df: pd.DataFrame) -> dict:
    ...
    if score == 3: q, lbl, tim = "strong", f"✅ ดีดกลับแข็งแกร่ง ({rsi_min:.1f} → +{rsi_rise:.1f})", "⭐ จังหวะเข้าซื้อดีที่สุด"
    elif score == 2: q, lbl, tim = "moderate", f"🟡 ดีดกลับปานกลาง ({rsi_min:.1f} → +{rsi_rise:.1f})", "⚡ พิจารณาเข้าซื้อได้"
    elif score == 1: q, lbl, tim = "weak", f"🟠 ดีดกลับอ่อน (+{rsi_rise:.1f} จุด)", "⚠️ ยังไม่แนะนำ"
    else: q, lbl, tim = "none", f"⬜ ยังไม่ดีดกลับ ({rsi_min:.1f})", "🚫 รอ RSI ดีดกลับก่อน"
...
def find_order_blocks(df: pd.DataFrame, lookback: int = OB_LOOKBACK) -> dict:
    ob = {"bullish_ob_price": None, "bearish_ob_price": None, "has_bullish_ob": False, "has_bearish_ob": False}
    if len(df) < lookback + 5: return ob
...
def check_bullish_divergence(df: pd.DataFrame) -> bool:
    ...
    prev_low_p, prev_low_rsi = prev["low"].iloc[min_idx], prev["RSI"].iloc[min_idx]
...
# ==========================================
# Market Scanner Pipeline Base Engine
# ==========================================
def scan_market():
    ...
    logger.info("Phase 1: Bulk fetch 4H data from Binance Multi-Cluster Gateways...")
    all_4h = bulk_fetch_4h(COINS)

    logger.info("Phase 2: Bulk fetch 1D data from Binance Multi-Cluster Gateways...")
    all_1d = bulk_fetch_1d(COINS)

    logger.info("Phase 3: วิเคราะห์ภาพรวมตลาดเบื้องต้น (BTC)...")
    ...
    if btc_df is not None:
        ...
        market_regime = "Bull Market 🟢 (BTC > 4H EMA200)" if btc_price > btc_ema200 else "Bear Market 🔴 (BTC < 4H EMA200)"

    logger.info("Phase 4: สแกนสัญญาณทางเทคนิคเชิงลึกรายเหรียญ...")
    for coin in COINS:
        df        = all_4h.get(coin)
        ...
        current_price, rsi = row["close"], row["RSI"]
        ema_50, ema_200, atr, adx = row["EMA_50"], row["EMA_200"], row["ATR"], row["ADX"]
        ...
        fibo_4h_max, fibo_4h_min = df["high"].iloc[-60:].max(), df["low"].iloc[-60:].min()
        ...
        if current_price > ema_200:
            ...
            if in_fibo_zone or in_4h_fibo_zone or in_ob_zone or in_fvg_zone:
                if current_price > (ema_50 * 0.98) and (rsi <= RSI_OVERSOLD or rsi <= RSI_PULLBACK_THRESHOLD):
                    if bounce_info["quality"] in ["strong", "moderate"] and candle_info["reversal_strength"] >= 1:
            ...
            if in_deep_support and is_divergence: signal_type = f"🚨 DEEP REVERSAL + Bullish Div 🐳{vol_tag}"
            elif in_deep_support and bounce_info["quality"] == "strong": signal_type = f"🛡️ Deep Support Strong Bounce 📉{vol_tag}"
            elif in_fibo_zone or in_ob_zone:
                if rsi <= RSI_OVERSOLD: signal_type = f"Deep Retracement Buy (เสี่ยงสูง) 📉{vol_tag}"
                elif is_divergence: signal_type = f"Macro Support Divergence 📈{vol_tag}"

        if signal_type:
            sig_score, score_label = calculate_signal_score(
                rsi=rsi, bounce_info=bounce_info, candle_info=candle_info, vol_confirmed=vol_confirmed,
                in_fibo_zone=(in_fibo_zone or in_4h_fibo_zone), in_ob_zone=in_ob_zone, in_fvg_zone=in_fvg_zone,
                weekly_ctx=weekly_ctx, mtf_info=mtf_info, adx=adx, is_divergence=is_divergence, trend_info=trend_info,
            )
            if sig_score < MINIMUM_SIGNAL_SCORE: signal_type = ""

        if signal_type:
            onchain_ctx = analyze_onchain_momentum(coin) if coin in ONCHAIN_SUPPORTED_COINS else _empty_onchain()
            futures_ctx = analyze_futures_context(coin)

            if onchain_ctx.get("onchain_warning"): signal_type = f"⚠️ On-Chain เสี่ยงสูง + {signal_type}"
            if weekly_ctx.get("weekly_bullish_div"): signal_type = f"⭐ {signal_type} + [1W Bullish Div]"
            elif weekly_ctx.get("rsi_weekly") and weekly_ctx["rsi_weekly"] <= 35: signal_type = f"💎 {signal_type} + [1W Oversold]"

            entry_min, entry_max = format_price(current_price * 0.98), format_price(current_price * 1.01)
            target_tp1, target_tp2 = current_price * (1 + tp1_pct), current_price * (1 + tp2_pct)
            ...
            buy_signals.append({
                "coin": coin, "trend": coin_trend, "price": format_price(current_price), "rsi": rsi_rounded, "type": signal_type,
                "entry":  f"${entry_min} - ${entry_max}", "tp1": f"${format_price(target_tp1)} (+{tp1_pct*100:.0f}%)", "tp2": f"${format_price(target_tp2)} (+{tp2_pct*100:.0f}%)",
                "dynamic_tp": f"${format_price(dynamic_tp_ob)}", "trailing_stop": f"${format_price(max(trailing_stop_val, sl_val))}", "sl": f"${format_price(sl_val)}",
                "vol_confirmed": vol_confirmed, "trend_info": trend_info, "bounce_info": bounce_info, "ob_info": ob_info, "fvg_info": fvg_info, "candle_info": candle_info, "mtf_info": mtf_info,
                "sig_score": sig_score, "score_label": score_label, "weekly_ctx": weekly_ctx, "monthly_ctx": monthly_ctx, "cycle_ctx": cycle_ctx, "onchain_ctx": onchain_ctx, "death_cross_ctx": death_cross_ctx, "futures_ctx": futures_ctx,
                "corr_btc": corr_btc, "squeeze_warning": squeeze_warning, "adx": round(adx, 2), "atr_pct": round(atr_pct, 2), "dyn_multiplier": dynamic_multiplier, "pos_size": f"${pos_size:.2f}", "sl_risk_pct": f"{sl_distance_pct*100:.1f}%",
            })

        if rsi >= RSI_OVERBOUGHT or ob_info["has_bearish_ob"]:
            tp_min, tp_max = format_price(current_price * 1.00), format_price(current_price * (1 + tp1_pct * 0.4))
            ...
            sell_signals.append({
                ...
                "tp_zone": f"${tp_min} - ${tp_max}", "exit": f"${format_price(exit_val)}", "vol_confirmed": vol_confirmed, "trend_info": trend_info, "ob_info": ob_info, "weekly_ctx": weekly_ctx, "cycle_ctx": cycle_ctx,
            })

    if total_valid_coins > 0:
        bullish_ratio = (bullish_coins / total_valid_coins) * 100
        summary_msg   = (
            f"📊 <b>[Market Summary – 🤖 Binance Multi-Source Bypass Engine]</b>\n"
            f"ดัชนีหลักภาพรวม (BTC): <b>{market_regime}</b>\n"
            f"ขนาดทุนในระบบ: {PORTFOLIO_USDT} USDT | Risk Control: {RISK_PER_TRADE_PCT}%/ไม้\n"
            f"📈 จำนวนเหรียญขาขึ้น: {bullish_coins} | 📉 เหรียญขาลง: {bearish_coins} เหรียญ\n"
        )
        if bullish_ratio >= 65: summary_msg += "🔥 สภาพตลาดรวม: <b>🟢 Strong Bullish (เปิดหน้าซื้อได้เปรียบ)</b>"
        elif bullish_ratio >= 40: summary_msg += "🔥 สภาพตลาดรวม: <b>🟡 Sideways (เน้นเล่นในกรอบแคบ)</b>"
        else: summary_msg += "🔥 สภาพตลาดรวม: <b>🔴 Bearish (ตลาดมีความเสี่ยงสูง)</b>"
        summary_msg += "\n\n📋 <b>สรุปแนวโน้มรายตัว:</b>\n" + "\n".join(coin_trends_summary)
    else:
        summary_msg = "⚠️ ขออภัย ระบบไม่สามารถเชื่อมต่อดึงข้อมูลเหรียญในรอบนี้ได้"

    return buy_signals, sell_signals, summary_msg

def _empty_weekly(): return {"rsi_weekly": None, "weekly_bullish_div": False, "weekly_status_label": "↔️ ไม่มีข้อมูลสัปดาห์", "fibo_618": None, "fibo_786": None, "fibo_886": None, "liquidity_pool": None, "psycho_support": None}
def _empty_monthly(): return {"m_resistance_target": None, "m_support_target": None, "monthly_summary_label": "⏳ ไม่มีข้อมูลเดือน", "monthly_trend": "sideways"}
def _empty_cycle(): return {"cycle_target_zone": None, "cycle_confluence_factors": [], "cycle_summary_label": "⏳ ไม่มีข้อมูลไซเคิล"}
def _empty_dc(): return {"has_death_cross": False, "death_cross_label": "⚪ 1D Death Cross: ปลอดภัย"}
def _empty_onchain(): return {"onchain_warning": False, "onchain_label": "🔹 On-Chain: ข้าม", "large_tx_change_pct": 0.0}

# ==========================================
# Telegram Message Formatter & Orchestrator
# ==========================================
def build_messages(buy_list: list, sell_list: list, market_summary: str) -> list:
    blocks = [market_summary]
    if buy_list:
        header = "🎯 <b>[Crypto Screener 4H – สัญญาณเปิดสถานะซื้อ]</b>"
        current = header
        for opt in buy_list:
            vol_note = "\n🔊 Volume: <b>ยืนยันสัญญาณ (สูงกว่า MA20)</b>" if opt["vol_confirmed"] else "\n🔇 Volume: ต่ำ (ไม่ยืนยัน)"
            corr_alert = f"\n⚠️ <b>Correlation BTC:</b> {opt['corr_btc']} (ระวังวิ่งตามกัน)" if opt.get("corr_btc", 0) > 0.85 and opt["coin"] != "BTC" else ""
            time_alert = f"\n⏳ <b>Squeeze Warning ({opt.get('adx',0)}):</b> ตลาดพักตัวบีบอัด" if opt.get("squeeze_warning") else ""
            
            ti, bi, ob, fvg, ci, mtfi = opt["trend_info"], opt["bounce_info"], opt["ob_info"], opt["fvg_info"], opt["candle_info"], opt["mtf_info"]
            w_ctx, m_ctx, c_ctx, oc, dc, ft = opt["weekly_ctx"], opt["monthly_ctx"], opt["cycle_ctx"], opt["onchain_ctx"], opt["death_cross_ctx"], opt["futures_ctx"]

            confluence = "\n🛡️ <b>แนวรับ Confluence คีย์สำคัญ:</b>"
            ...
            if fvg.get("has_fvg_support"):
                confluence += f"\n   ⚡ กรอบ FVG (4H): <code>${format_price(fvg['fvg_bottom'])} - ${format_price(fvg['fvg_top'])}</code>"
            if ob.get("has_bullish_ob"):
                confluence += f"\n   🐳 วอลุ่มหนา OB Support: <code>${format_price(ob['bullish_ob_price'])}</code>"

            coin_msg = (
                f"\n\n🪙 <b>{opt['coin']}</b> | ราคาปัจจุบัน: ${opt['price']}"
                f"\n🚨 รูปแบบสัญญาณ: <b>{opt['type']}</b> | RSI: {opt['rsi']}"
                f"{vol_note}{corr_alert}{time_alert}\n🏆 ประเมินเกรด: {opt['score_label']}"
                f"\n📡 โครงสร้าง MTF: {mtfi.get('mtf_label','')}\n🕯️ แท่งเทียน: {ci['reversal_label']}"
                f"{confluence}\n📐 แนวโน้มอินดิเคเตอร์: {ti['trend_label']}\n🔄 แรงดีดตัว: {bi['quality_label']}"
                f"\n🗓️ กรอบเวลาใหญ่: {w_ctx.get('weekly_status_label','')}\n📈 ตลาดอนุพันธ์: {ft.get('futures_label','')}{dc.get('death_cross_label','')}"
                f"\n\n🛡️ <b>[แผนควบคุมความเสี่ยง - Risk Management]</b>"
                f"\n💼 ขนาดไม้ที่ควรเข้า: <code>{opt['pos_size']}</code> | ระยะปลอดภัยตัดขาดทุน: {opt['sl_risk_pct']}"
                f"\n❌ จุดยอมแพ้เด็ดขาด (Hard SL): <code>{opt['sl']}</code>"
                f"\n🚀 <b>[เป้าหมายทำกำไร - Take Profit Tiers]</b>"
                f"\nTarget 1: <code>{opt['tp1']}</code> | Target 2: <code>{opt['tp2']}</code>"
                f"\n🔥 เป้าหมายรันเทรนด์สุดซอย (Dynamic TP เมื่อ RSI=70): <code>{opt['dynamic_tp']}</code>"
            )
            if len(current) + len(coin_msg) > 3500:
                blocks.append(current); current = header + coin_msg
            else: current += coin_msg
        blocks.append(current)

    if sell_list:
        header  = "⚠️ <b>[Crypto Screener 4H – สัญญาณเข้าโซนขายระวังตัว]</b>"
        current = header
        for opt in sell_list:
            coin_msg = f"\n\n🪙 <b>{opt['coin']}</b> | ราคาปัจจุบัน: ${opt['price']} | RSI 4H: {opt['rsi']}\n🔴 โซนทยอยล็อกกำไร: <code>{opt['tp_zone']}</code>\n❌ จุดขายหนีตตาย (Safety Exit): <code>{opt['exit']}</code>"
            if len(current) + len(coin_msg) > 3500:
                blocks.append(current); current = header + coin_msg
            else: current += coin_msg
        blocks.append(current)
...
def verify_network_environment():
    try:
        ...
        logger.info(f"🌐 [Network Security] Python Engine กำลังวิ่งผ่าน IP: {ip} (VPN Secured)")
    except Exception as e:
        logger.warning(f"🌐 [Network Security] ไม่สามารถตรวจสอบ IP ได้ ({e}) แต่จะดำเนินการต่อ...")
...
if __name__ == "__main__":
    logger.info("=" * 60)
    logger.info("Crypto Screener v3.6 – Active Engine Running (Dynamic Multi-Source Config)")
    logger.info("=" * 60)

    # เช็ก IP ว่ามุดท่อสำเร็จหรือไม่
    ...
    logger.info("ระบบทำงานสมบูรณ์ สิ้นสุดกระบวนการตรวจสอบ!")
