...
# หมายเหตุ: สำหรับดึง On-Chain ข้อมูลวาฬบางส่วนเท่านั้น (หากพัง ระบบจะข้ามให้อัตโนมัติ บอทไม่หยุดทำงาน)
...
# ==========================================
API_RATE_LIMIT_DELAY = 0.5   # หน่วงเวลาปลอดภัยสำหรับ Binance API
API_MAX_RETRIES      = 3
...
# --- Binance Cluster Gateways (สำหรับแก้ปัญหา Geo-blocking 451 และ 404) ---
BINANCE_ENDPOINTS = [
...
api_session.headers.update({
    "User-Agent": "CryptoScreenerBot/3.5 (Binance Dynamic Multi-Cluster Engine)",
...
# ==========================================
# Data Fetching (ระบบ Dynamic Gateway Fallback)
# ==========================================
def get_historical_data(coin: str) -> pd.DataFrame | None:
    """ดึงข้อมูลแท่งเทียน 4H พร้อมระบบหมุนเวียน Gateway หลบเลี่ยง Error 451/404"""
    symbol = f"{coin}USDT"
    params = {
        "symbol": symbol,
        "interval": "4h",
    ...
    # วนลูปยิงหาแต่ละ Gateway เผื่อตัวใดตัวหนึ่งล่มหรือบล็อก IP
    for base_url in BINANCE_ENDPOINTS:
        url = f"{base_url}/api/v3/klines"
        try:
            resp = api_session.get(url, params=params, timeout=12)
            if resp.status_code == 200:
                data = resp.json()
    ...
    logger.error(f"❌ {coin} 4H ดึงข้อมูลไม่สำเร็จหลังจากลองครบทุก Gateway แล้ว")
    return None


def get_histoday_data(coin: str) -> pd.DataFrame | None:
    """ดึงข้อมูลแท่งเทียน 1D พร้อมระบบหมุนเวียน Gateway หลบเลี่ยง Error 451/404"""
    symbol = f"{coin}USDT"
    params = {
        "symbol": symbol,
        "interval": "1d",
    ...
    logger.error(f"❌ {coin} 1D ดึงข้อมูลไม่สำเร็จหลังจากลองครบทุก Gateway แล้ว")
    return None
...
# ==========================================
# On-Chain Metric (สแตนด์บายระบบ Fallback)
# ==========================================
...
# ==========================================
# Futures Core Engine (ดึงข้อมูลดิบสาธารณะจาก Binance)
# ==========================================
...
def scan_market():
    ...
    if total_valid_coins > 0:
        ...
        summary_msg   = (
            f"📊 <b>[Market Summary – 🤖 Binance Multi-Cluster Engine]</b>\n"
...
if __name__ == "__main__":
    logger.info("=" * 60)
    logger.info("Crypto Screener v3.5 – Active Engine Running (Dynamic Cluster Config)")
    logger.info("=" * 60)
