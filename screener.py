...
# หมายเหตุ: ใช้สำหรับดึง On-Chain และใช้เป็นท่อสำรองเมื่อตระกูล Binance โดน Geo-Block
...
# --- Binance Cluster Gateways (สำหรับดึงข้อมูลทางตรง) ---
BINANCE_ENDPOINTS = [
...
# ==========================================
# Data Fetching (ระบบ Multi-Source Bypass Geo-block ด้วย Isolated Request)
# ==========================================
def get_historical_data(coin: str) -> pd.DataFrame | None:
    """ดึงข้อมูลแท่งเทียน 4H พร้อมระบบหมุนเวียน Gateway และสลับ Source สำรองแยก Session อิสระเพื่อหลบเลี่ยงการจำค่าบล็อก"""
    symbol = f"{coin}USDT"
    params = {
        "symbol": symbol,
        "interval": "4h",
    ...
    backup_params = {
        ...
        "tsym": "USDT",
        "limit": BINANCE_LIMIT,
        "e": "Binance",     # อ้างอิงราคากระดาน Binance ตรง 100%
        "aggregate": 4      # มัดรวม 4 ชั่วโมง = แท่ง 4H
    }
    if CRYPTOCOMPARE_API_KEY:
        backup_params["api_key"] = CRYPTOCOMPARE_API_KEY

    try:
        # ใช้ requests สดๆ ไม่แชร์ session ร่วมกับพาร์ทด้านบน เพื่อล้างคราบไคลคุกกี้ที่โดน Block 451
        resp = requests.get(backup_url, params=backup_params, headers={"User-Agent": "Mozilla/5.0"}, timeout=12)
        if resp.status_code == 200:
            res_json = resp.json()
            if res_json.get("Response") == "Success" and "Data" in res_json and "Data" in res_json["Data"]:
                raw_list = res_json["Data"]["Data"]
                if len(raw_list) > 0:
                    df = pd.DataFrame(raw_list)
                    
                    # บังคับแปลงคอลัมน์และตั้งค่า Index คืนรูปกลับสู่โมเดลบอทหลัก
                    df["time"] = pd.to_datetime(df["time"], unit="s")
                    df.set_index("time", inplace=True)
                    
                    if "volumeto" not in df.columns and "volume" in df.columns:
        ...
        logger.error(f"❌ ท่อกลางสำรอง 4H เกิดข้อผิดพลาดด้านโครงสร้างข้อมูล: {e}")
...
def get_histoday_data(coin: str) -> pd.DataFrame | None:
    """ดึงข้อมูลแท่งเทียน 1D พร้อมระบบหมุนเวียน Gateway และสลับ Source สำรองแยก Session อิสระเพื่อความเสถียรสูงสุด"""
    symbol = f"{coin}USDT"
    params = {
        "symbol": symbol,
        "interval": "1d",
    ...
    try:
        ...
        logger.error(f"❌ ท่อกลางสำรอง 1D เกิดข้อผิดพลาดด้านโครงสร้างข้อมูล: {e}")
