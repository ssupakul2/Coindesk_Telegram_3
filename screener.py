...
def _fetch_live_price_binance(coin: str) -> float | None:
    ...
    for base_url in ["https://api.binance.com", "https://data-api.binance.vision"]:
        try:
            ...
            continue
    # Fallback: CoinGecko (ช้ากว่าแต่ universal)
    return _fetch_realtime_price_coingecko(coin)
...
def scan_market(positions: dict):
    ...
    for coin in COINS:
        ...
        if price > ema200:
            ...
            #
            # [Approach 2] Live Price Check: ก่อนจะ trigger "ราคาหลุด EMA200"
            # ตรวจราคาจริง ณ ขณะนี้ (ไม่ใช่ close ของแท่ง 4H ที่ยังไม่ปิด)
            # ถ้าราคาจริงได้ทะลุกลับเหนือ EMA200 แล้ว → ไม่ trigger severe alert
            # ป้องกันการเตือนผิดพลาดเมื่อแท่งปัจจุบันกำลังเขียวขึ้นมา
            # แต่ indicator ยังคำนวณจากแท่งก่อนหน้าที่ปิดแล้วอยู่ใต้ EMA200
            # ─────────────────────────────────────────────────────────────
            if coin in positions:
                near_ob_target = False
                est_target = estimate_price_for_target_rsi(df)
                ...
                # ราคาหลุด EMA200 (จาก closed candle) — ตรวจ live price ก่อนสรุป
                is_below_ema200_closed = price < ema200
                is_below_ema200_live   = is_below_ema200_closed  # default = เหมือนกัน ถ้า live check ล้มเหลว
                live_price = None
                ...
                if is_below_ema200_closed:
                    ...
                    if live_price is not None and live_price > 0:
                        ...
                        if not is_below_ema200_live:
                            # ราคาจริงทะลุกลับเหนือ EMA200 แล้ว แม้ closed candle ยังไม่ยืนยัน
                            logger.info(
                                f"✅ {coin}: closed candle ({format_price(price)}) < EMA200 "
                                f"แต่ live price ({format_price(live_price)}) > EMA200 — ไม่ trigger severe alert"
                            )

                # บวกคะแนนเฉพาะกรณีที่ทั้ง closed candle และ live price ยืนยันตรงกันว่าหลุดจริง
                if is_below_ema200_closed and is_below_ema200_live:
                    ...
                    # ลดความรุนแรงลง — แจ้งเป็นข้อสังเกตแทน severe alert
                ...
                # ใช้ is_below_ema200_live (ไม่ใช่ is_below_ema200_closed) เป็นเงื่อนไข force-include
                if exit_score_b >= eff_threshold_b or is_below_ema200_live:
                    ...
                    exit_watch_signals.append({
                        ...
                        "below_ema200_position": is_below_ema200_live,  # flag พิเศษสำหรับ Telegram
...
def build_messages(buy_list, exit_watch_list, position_updates, market_summary, supply_zone_alerts=None) -> list:
    ...
    if exit_watch_list:
        ...
        for opt in sorted(exit_watch_list, key=lambda x: -x["exit_score"]):
            ...
            msg = (
                f"\n\n🪙 <b>{opt['coin']}</b> | ราคา: ${opt['price']} | RSI: {opt['rsi']}{ext_line}{live_price_line}\n"
                f"{rsi_header}"
                f"{opt['exit_label']}\n"
