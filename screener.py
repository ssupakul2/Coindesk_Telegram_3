...
def _parse_binance_klines(data: list, coin: str, tf: str) -> pd.DataFrame | None:
    try:
        ...
        if hours_old > DATA_STALENESS_SKIP_HOURS:
...
def _check_df_staleness(df: pd.DataFrame, coin: str, source: str) -> float:
    ...
    try:
        ...
        if hours_old > DATA_STALENESS_WARN_HOURS:
            logger.warning(
                f"⚠️ {coin} {source}: candle ล่าสุดเก่า {hours_old:.1f}h "
                f"(threshold warn={DATA_STALENESS_WARN_HOURS}h skip={DATA_STALENESS_SKIP_HOURS}h)"
            )
        return hours_old
    except Exception:
        return 999.0
...
def _fetch_4h_from_kucoin(coin: str) -> pd.DataFrame | None:
    ...
    try:
        ...
        if hours_old > DATA_STALENESS_SKIP_HOURS:
            logger.error(
                f"❌ {coin} CoinGecko OHLC เก่าเกินไป ({hours_old:.1f}h > {DATA_STALENESS_SKIP_HOURS}h) "
...
def get_historical_data(coin: str) -> pd.DataFrame | None:
    ...
    2. CryptoCompare   — fallback สำหรับเหรียญไม่มีบน Binance
    3. Gate.io Spot    — fallback สำหรับเหรียญใหม่/เล็กที่มีบน Gate.io (เช่น EIGEN)
    4. KuCoin Spot     — fallback เพิ่มเติม
    5. CoinGecko market_chart — fallback สุดท้าย (hourly data resample → 4H, สดกว่า /ohlc)
    """
    symbol   = f"{coin}USDT"
...
def get_volume_filter_info(coin: str, volume_data: dict) -> dict:
    ...
    if vol is None:
        # No data -> fail open (don't block on missing data)
        return info
...
def scan_market(positions: dict):
    ...
    for coin in COINS:
        ...
        if df is None or len(df) < EMA_LONG + 10:
            if df is None and coin in positions:
                stale_coins.append(f"{coin}(ไม่มีข้อมูล)")
            continue

        # Check staleness of the df we received — log for summary
        ...
        # RSI > 95 ที่ไม่ใช่ Binance data = สัญญาณ frozen price (ราคาเดิมทุก candle)
        # → diff() ทุกแท่ง = 0 → avg_loss = 0 → RSI = fillna(100)
        # ตรวจโดยดูว่า close prices ใน 10 candle ล่าสุดเหมือนกันหรือเปล่า
        if rsi > 95:
            recent_closes = df["close"].iloc[-10:]
            price_variance = recent_closes.std()
            if price_variance < price * 0.0001:  # std < 0.01% of price = frozen
                logger.warning(
                    f"⚠️ {coin}: RSI={rsi:.1f} + price variance={price_variance:.8f} "
                    f"→ ข้อมูลแช่แข็ง (frozen price) — ลองดึงข้อมูลใหม่จาก real-time"
                )
                # พยายาม patch ด้วย real-time price จาก CoinGecko simple/price
                rt_price = _fetch_realtime_price_coingecko(coin)
                if rt_price and abs(rt_price - price) / price > 0.001:
                    # ราคา real-time ต่างจาก frozen price > 0.1% → patch และ recalculate
                    df = _patch_df_with_realtime_price(df, coin, rt_price)
                    df = calculate_indicators(df)
                    row = df.iloc[-1]
                    price, rsi, ema50, ema200 = row["close"], row["RSI"], row["EMA_50"], row["EMA_200"]
                    atr, adx, vol_confirmed = row["ATR"], row["ADX"], is_volume_confirmed(row)
                    logger.info(f"✅ {coin}: patched with real-time price ${rt_price:.6f}, RSI now {rsi:.1f}")
                    stale_coins.append(f"{coin}(patched→${format_price(rt_price)})")
                else:
                    logger.error(f"❌ {coin}: ข้อมูลแช่แข็ง + real-time patch ล้มเหลว — ข้าม coin นี้")
                    stale_coins.append(f"{coin}(frozen-skipped)")
                    continue  # ข้าม coin นี้ทั้งหมด ไม่ให้สัญญาณ
        # ─────────────────────────────────────────────────────────────────
        ...
        if price > ema200:
            ...
            # [Soft Override] RSI >= RSI_EXIT_WARN_THRESHOLD (70) -> ลด effective threshold เป็น 25
            ...
            if rsi >= RSI_EXIT_HARD_OVERRIDE:
                ...
                effective_exit_threshold = 25  # lower threshold สำหรับ RSI 70-77
