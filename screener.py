...
def _is_df_frozen(df: pd.DataFrame, coin: str = "") -> bool:
    """
    ตรวจ frozen data ด้วย Coefficient of Variation
    CV < 0.0002 และ unique values น้อยกว่า 20% ของ bars = frozen
    """
    ...
    if frozen:
        logger.warning(f"🧊 {coin}: frozen data detected (cv={cv:.6f}, unique={n_unique}/20)")
    return frozen
