...
# หมายเหตุ: ใช้สำหรับดึง On-Chain ข้อมูลวาฬบางส่วนเท่านั้น (หากพัง ระบบจะข้ามให้อัตโนมัติ บอทไม่หยุดทำงาน)
...
# ==========================================
# Data Fetching (ระบบ Multi-Source Bypass Geo-block)
# ==========================================
def get_historical_data(coin: str) -> pd.DataFrame | None:
    """ดึงข้อมูลแท่งเทียน 4H พร้อมระบบหมุนเวียน Gateway และสลับ Source สำรองเมื่อเจอ 451/404 Geo-block"""
    symbol = f"{coin}USDT"
    params = {
        "symbol": symbol,
        "interval": "4h",
    ...
    # ทางเลือกที่ 2: (Bypass Engine) หากติดบล็อก IP ระดับประเทศ (451/404) ให้สลับมาใช้ Public Global Aggregator ทันที
    ...
    backup_params = {
        ...
        "tsym": "USDT",
        "limit": BINANCE_LIMIT,
        "e": "Binance",     # เจาะจงดึงข้อมูลเฉพาะของตลาด Binance มาคำนวณ ความแม่นยำ 100%
        "aggregate": 4      # มัดรวมแท่งละ 1 ชั่วโมง จำนวน 4 แท่ง = แท่ง 4H พอดีตามโครงสร้างบอท
    }
    try:
        resp = api_session.get(backup_url, params=backup_params, timeout=12)
        if resp.status_code == 200:
            res_json = resp.json()
            if res_json.get("Response") == "Success" and "Data" in res_json:
        ...
        logger.error(f"❌ ท่อกลางสำรอง 4H เกิดข้อผิดพลาด: {e}")
...
def get_histoday_data(coin: str) -> pd.DataFrame | None:
    """ดึงข้อมูลแท่งเทียน 1D พร้อมระบบหมุนเวียน Gateway และสลับ Source สำรองเมื่อเจอ 451/404 Geo-block"""
    symbol = f"{coin}USDT"
    params = {
        "symbol": symbol,
        "interval": "1d",
    ...
    try:
        ...
        logger.error(f"❌ ท่อกลางสำรอง 1D เกิดข้อผิดพลาด: {e}")
...
# ==========================================
# Math & Mathematical Engineering Formulas
# ==========================================
...
# ==========================================
# Timeframe Context Analyzers (1W / 1M / Cycle)
# ==========================================
