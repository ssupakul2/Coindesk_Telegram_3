...
# จำนวนแท่งที่ขอจากแต่ละ source
BINANCE_LIMIT        = 500   # แท่ง 4H/1D จาก Binance
CC_HISTOHOUR_LIMIT   = 2000  # แท่ง 1H จาก CryptoCompare → resample เป็น ~500 แท่ง 4H
CC_HISTODAY_LIMIT    = 500   # แท่ง 1D จาก CryptoCompare

CACHE_TTL_SECONDS    = 3600

# --- Binance Cluster Gateways ---
BINANCE_ENDPOINTS = [
...
api_session.headers.update({
    "User-Agent": "CryptoScreenerBot/3.7 (Binance Dynamic Multi-Source Engine)",
...
# ==========================================
# Data Fetching — แก้ไขหลัก (v3.7)
# ==========================================

def _parse_binance_klines(data: list, coin: str, tf: str) -> pd.DataFrame | None:
    """แปลง raw klines list จาก Binance → DataFrame พร้อมใช้"""
    try:
...
def _fetch_from_binance(symbol: str, interval: str, limit: int,
                        coin: str, tf: str) -> pd.DataFrame | None:
    """วนลูปลอง Binance endpoints ทั้งหมด"""
...
def _fetch_4h_from_cryptocompare(coin: str) -> pd.DataFrame | None:
    """
    Fallback 4H: ดึง histohour (1H บาร์) จาก CryptoCompare แล้ว resample เป็น 4H

    สิ่งที่แก้ไขจากเวอร์ชันเดิม:
      1. tsym "USDT" → "USD"   : CryptoCompare ใช้ USD เป็นหลัก USDT มักไม่มีข้อมูล
      2. ลบ "e": "Binance"     : ถ้า Binance geo-block parameter นี้ทำให้ CC ก็ดึงไม่ได้ด้วย
      3. ลบ "aggregate": 4     : histohour ส่งคืน 1H เสมอ aggregate ไม่มีผล
      4. limit 2000 (1H bars)  : resample เองด้วย pandas ได้ ~500 แท่ง 4H
    """
    url    = "https://min-api.cryptocompare.com/data/v2/histohour"
    params = {
        "fsym":  coin,
        "tsym":  "USD",          # ← แก้จาก USDT
        "limit": CC_HISTOHOUR_LIMIT,  # ← 2000 แท่ง 1H
        # ไม่มี "e": "Binance"   ← ลบออก
        # ไม่มี "aggregate": 4   ← ลบออก
    }
    ...
    try:
        ...
        if resp.status_code != 200:
            logger.error(f"❌ {coin} 4H CC HTTP {resp.status_code}")
            return None

        res_json = resp.json()
        if res_json.get("Response") == "Error":
            logger.error(f"❌ {coin} 4H CC ปฏิเสธ: {res_json.get('Message')}")
            return None

        raw = res_json.get("Data", {}).get("Data")
        if not raw:
            logger.error(f"❌ {coin} 4H CC ไม่พบ Data.Data")
            return None

        df = pd.DataFrame(raw)
        logger.info(f"🔍 {coin} 4H CC columns: {list(df.columns)}")

        # ตั้ง index เวลา (unit=s สำหรับ CryptoCompare)
        df["time"] = pd.to_datetime(df["time"], unit="s")
        df.set_index("time", inplace=True)

        # map volumeto — CryptoCompare ตั้งชื่อต่างกัน
        ...
        # resample 1H → 4H ด้วย pandas (สำคัญมาก)
        ...
        if len(df_4h) < 10:
            logger.error(f"❌ {coin} 4H CC resample ได้แค่ {len(df_4h)} แท่ง")
            return None

        logger.info(f"✅ {coin} 4H CC fallback สำเร็จ ({len(df_4h)} แท่ง 4H)")
        return df_4h

    except Exception as e:
        logger.error(f"❌ {coin} 4H CC error: {e}", exc_info=True)
        return None


def _fetch_1d_from_cryptocompare(coin: str) -> pd.DataFrame | None:
    """
    Fallback 1D: ดึง histoday จาก CryptoCompare

    สิ่งที่แก้ไขจากเวอร์ชันเดิม:
      1. tsym "USDT" → "USD"
      2. ลบ "e": "Binance" ออก
      3. limit เพิ่มเป็น CC_HISTODAY_LIMIT (500 วัน)
    """
    url    = "https://min-api.cryptocompare.com/data/v2/histoday"
    params = {
        "fsym":  coin,
        "tsym":  "USD",          # ← แก้จาก USDT
        "limit": CC_HISTODAY_LIMIT,  # ← 500 วัน
        # ไม่มี "e": "Binance"   ← ลบออก
    }
    if CRYPTOCOMPARE_API_KEY:
        params["api_key"] = CRYPTOCOMPARE_API_KEY
    ...
    try:
        ...
        if resp.status_code != 200:
            logger.error(f"❌ {coin} 1D CC HTTP {resp.status_code}")
            return None

        res_json = resp.json()
        if res_json.get("Response") == "Error":
            logger.error(f"❌ {coin} 1D CC ปฏิเสธ: {res_json.get('Message')}")
            return None

        raw = res_json.get("Data", {}).get("Data")
        if not raw:
            logger.error(f"❌ {coin} 1D CC ไม่พบ Data.Data")
            return None

        df = pd.DataFrame(raw)
        logger.info(f"🔍 {coin} 1D CC columns: {list(df.columns)}")
        ...
        if len(df_out) < 10:
            logger.error(f"❌ {coin} 1D CC ได้แค่ {len(df_out)} แถว")
            return None

        logger.info(f"✅ {coin} 1D CC fallback สำเร็จ ({len(df_out)} วัน)")
        return df_out

    except Exception as e:
        logger.error(f"❌ {coin} 1D CC error: {e}", exc_info=True)
        return None


def get_historical_data(coin: str) -> pd.DataFrame | None:
    """ดึงข้อมูล 4H — ลอง Binance ก่อน fallback CryptoCompare"""
    symbol = f"{coin}USDT"

    # ทางเลือกที่ 1: Binance (ต้องได้ >= EMA_LONG+10 แท่ง)
    df = _fetch_from_binance(symbol, "4h", BINANCE_LIMIT, coin, "4H")
    if df is not None and len(df) >= EMA_LONG + 10:
        return df

    if df is not None:
        logger.warning(f"⚠️ {coin} 4H Binance ได้แค่ {len(df)} แท่ง (ต้องการ {EMA_LONG+10}+) → CC fallback")
    else:
        logger.warning(f"⚠️ {coin} 4H Binance ล้มเหลวทุก endpoint → CC fallback")

    # ทางเลือกที่ 2: CryptoCompare + resample
    return _fetch_4h_from_cryptocompare(coin)


def get_histoday_data(coin: str) -> pd.DataFrame | None:
    """ดึงข้อมูล 1D — ลอง Binance ก่อน fallback CryptoCompare"""
    symbol = f"{coin}USDT"

    # ทางเลือกที่ 1: Binance
    df = _fetch_from_binance(symbol, "1d", BINANCE_LIMIT, coin, "1D")
    if df is not None and len(df) >= 30:
        return df

    if df is not None:
        logger.warning(f"⚠️ {coin} 1D Binance ได้แค่ {len(df)} แท่ง → CC fallback")
    else:
        logger.warning(f"⚠️ {coin} 1D Binance ล้มเหลวทุก endpoint → CC fallback")

    # ทางเลือกที่ 2: CryptoCompare histoday
    return _fetch_1d_from_cryptocompare(coin)
...
def bulk_fetch_4h(coins: list) -> dict:
    ...
    if _cache_4h and (now - _cache_ts_4h) < CACHE_TTL_SECONDS:
        logger.info("ดึงข้อมูล 4H จาก Cache")
        return _cache_4h
    result = {}
    for coin in coins:
    ...
    _cache_4h    = result
    _cache_ts_4h = now
    logger.info(f"Bulk Fetch 4H เสร็จ: {len(result)}/{len(coins)} coins")
    return result
...
def bulk_fetch_1d(coins: list) -> dict:
    ...
    if _cache_1d and (now - _cache_ts_1d) < CACHE_TTL_SECONDS:
        logger.info("ดึงข้อมูล 1D จาก Cache")
        return _cache_1d
    result = {}
    for coin in coins:
    ...
    _cache_1d    = result
    _cache_ts_1d = now
    logger.info(f"Bulk Fetch 1D เสร็จ: {len(result)}/{len(coins)} coins")
    return result
...
def confirm_reversal_candle(df: pd.DataFrame) -> dict:
    ...
    if (body0 > 0 and lower_wick0 >= 2 * body0 and
            upper_wick0 <= 0.3 * body0 and c0["close"] > c0["open"]):
        result["has_hammer"] = True
        result["reversal_strength"] += 1

    if (c2["close"] < c2["open"] and body2 > 0 and body1 < body2 * 0.4 and
...
def get_correlation_adjusted_position(
    portfolio: float, risk_pct: float, sl_distance_pct: float,
    corr_btc: float, active_signals_count: int
) -> float:
...
def get_mtf_rsi_alignment(df_4h: pd.DataFrame, df_1d: pd.DataFrame) -> dict:
    result = {
        ...
        "rsi_4h": None, "rsi_1d": None, "mtf_label": "", "confluence_score": 0,
    }
    ...
    rsi_1d   = float((100 - 100 / (1 + rs)).iloc[-1]) if len(close_1d) > 0 else 50.0
...
def calculate_signal_score(
    ...
    bq = {"strong": 20, "moderate": 12, "weak": 5, "none": 0}.get(bounce_info.get("quality", "none"), 0)
    score += bq
    ...
    return score, f"{grade} | Score: {score}/100 | [{', '.join(factors)}]"
...
def analyze_weekly_context(df_1d: pd.DataFrame) -> dict:
    ...
    try:
        ...
        if len(prev_window) > 0:
            ...
            if prev_low_rsi <= RSI_BULL_DIV_MAX and curr_low < prev_low_p and curr_rsi_now > prev_low_rsi:
...
def analyze_cycle_targets(df_1d: pd.DataFrame) -> dict:
    ...
    try:
        ...
        curr_price = df_w["close"].iloc[-1]
        macro_high = df_w["high"].max()
        macro_low  = df_w["low"].min()
...
def check_death_cross_1d(df_1d: pd.DataFrame) -> dict:
    result = {"has_death_cross": False, "death_cross_label": "⚪ 1D Death Cross: ปลอดภัย"}
...
def analyze_onchain_momentum(coin: str) -> dict:
    ...
    try:
        ...
        if data.get("Response") == "Success" and isinstance(data.get("Data", {}).get("Data"), list):
            ...
            if len(df_oc) >= 15:
                tx_col = next(
                    (c for c in ["large_transaction_count", "transaction_count"] if c in df_oc.columns),
                    None
                )
                if tx_col:
        ...
        result["onchain_label"] = "🔹 On-Chain: ข้าม (API Limit)"
    return result


def analyze_futures_context(coin: str) -> dict:
    result = {"futures_warning": False, "futures_label": "📊 Futures (1M): ปกติ / ไม่มีข้อมูล"}
    symbol = f"{coin}USDT"
    try:
        fr_resp = api_session.get(
            f"https://fapi.binance.com/fapi/v1/premiumIndex?symbol={symbol}", timeout=10
        )
        oi_resp = api_session.get(
            f"https://fapi.binance.com/futures/data/openInterestHist?symbol={symbol}&period=1d&limit=30",
            timeout=10
        )
...
def scan_market():
    ...
    logger.info("Phase 1: Bulk fetch 4H...")
    all_4h = bulk_fetch_4h(COINS)

    logger.info("Phase 2: Bulk fetch 1D...")
    all_1d = bulk_fetch_1d(COINS)

    logger.info("Phase 3: Market Regime (BTC)...")
    ...
    if btc_df is not None:
        ...
        market_regime = (
            "Bull Market 🟢 (BTC > 4H EMA200)"
            if btc_price > btc_ema200
            else "Bear Market 🔴 (BTC < 4H EMA200)"
        )
    ...
    for coin in COINS:
        ...
        if df is None or len(df) < EMA_LONG + 10:
            logger.warning(f"{coin}: ข้อมูลไม่พอ (ได้ {len(df) if df is not None else 0} แท่ง) – ข้าม")
            continue
        ...
        if signal_type:
            ...
            if weekly_ctx.get("weekly_bullish_div"):
                ...
                signal_type = f"💎 {signal_type} + [1W Oversold]"
            ...
            buy_signals.append({
                ...
                "trend_info":      trend_info,    "bounce_info": bounce_info,
                "ob_info":         ob_info,       "fvg_info":    fvg_info,
                "candle_info":     candle_info,   "mtf_info":    mtf_info,
                "sig_score":       sig_score,     "score_label": score_label,
                "weekly_ctx":      weekly_ctx,    "monthly_ctx": monthly_ctx,
                "cycle_ctx":       cycle_ctx,     "onchain_ctx": onchain_ctx,
                "death_cross_ctx": death_cross_ctx, "futures_ctx": futures_ctx,
                "corr_btc":        corr_btc,      "squeeze_warning": squeeze_warning,
                "adx":             round(adx, 2), "atr_pct":    round(atr_pct, 2),
        ...
        if rsi >= RSI_OVERBOUGHT or ob_info["has_bearish_ob"]:
            ...
            sell_signals.append({
                ...
                "vol_confirmed": vol_confirmed,
                "trend_info": trend_info, "ob_info": ob_info,
    ...
    if total_valid_coins > 0:
        ...
        summary_msg   = (
            f"📊 <b>[Market Summary – Binance Multi-Source Engine v3.7]</b>\n"
...
def build_messages(buy_list: list, sell_list: list, market_summary: str) -> list:
    ...
    if buy_list:
        ...
        for opt in buy_list:
            ...
            corr_alert = (
                f"\n⚠️ <b>Correlation BTC:</b> {opt['corr_btc']} (ระวังวิ่งตามกัน)"
                if opt.get("corr_btc", 0) > 0.85 and opt["coin"] != "BTC" else ""
            )
            time_alert = (
                f"\n⏳ <b>Squeeze Warning ({opt.get('adx',0)}):</b> ตลาดพักตัวบีบอัด"
                if opt.get("squeeze_warning") else ""
            )

            ti, bi      = opt["trend_info"],  opt["bounce_info"]
            ob, fvg     = opt["ob_info"],     opt["fvg_info"]
            ci, mtfi    = opt["candle_info"], opt["mtf_info"]
            w_ctx       = opt["weekly_ctx"]
            m_ctx       = opt["monthly_ctx"]
            c_ctx       = opt["cycle_ctx"]
            oc, dc, ft  = opt["onchain_ctx"], opt["death_cross_ctx"], opt["futures_ctx"]
            ...
            coin_msg = (
                ...
                f"\n🔄 RSI Bounce: {bi['quality_label']}"
                f"\n🗓️ 1W: {w_ctx.get('weekly_status_label','')}"
                f"\n{ft.get('futures_label','')}"
                f"\n{dc.get('death_cross_label','')}"
                f"\n\n🛡️ <b>[Risk Management]</b>"
                f"\n💼 Position: <code>{opt['pos_size']}</code> | SL ระยะ: {opt['sl_risk_pct']}"
    ...
    if sell_list:
        header  = "⚠️ <b>[Crypto Screener 4H – สัญญาณโซนขาย]</b>"
        current = header
    ...
    if not buy_list and not sell_list:
        blocks.append("😴 <i>ตลาดนิ่ง: ไม่มีสัญญาณผ่าน Score Threshold ในรอบนี้</i>")
...
def verify_network_environment():
    try:
        ...
        logger.warning(f"🌐 ตรวจสอบ IP ไม่ได้: {e}")
...
if __name__ == "__main__":
    logger.info("=" * 60)
    logger.info("Crypto Screener v3.7 – Multi-Source Bypass Engine")
    logger.info("=" * 60)
