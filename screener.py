...
def get_historical_data(coin: str) -> pd.DataFrame | None:
    ...
    params = {"fsym": coin, "tsym": "USD", "limit": HISTOHOUR_LIMIT}
    try:
...
# ==========================================
# [NEW] Candle Pattern Confirmation (ข้อ 1)
# ==========================================
...
# ==========================================
# [NEW] Dynamic ATR Multiplier (ข้อ 2)
# ==========================================
def get_dynamic_atr_multiplier(tier: str, adx: float, atr_pct: float) -> float:
    """
    ปรับ SL multiplier ตาม volatility regime
    - ADX สูง = เทรนด์ชัด → SL หลวมขึ้น 20%
    - ATR > 4% ของราคา → SL หลวมขึ้น 15%
    - ADX ต่ำมาก (sideways) → SL แน่นลง 15%
    """
...
# ==========================================
# [NEW] Correlation-Adjusted Position Sizing (ข้อ 3)
# ==========================================
def get_correlation_adjusted_position(
    ...
    """
    ลด position size เมื่อ:
    - Correlation กับ BTC สูง (>0.85) → ลด 30%
    - มีหลาย signal พร้อมกัน → จำกัด total exposure
    """
...
# ==========================================
# [NEW] Multi-Timeframe RSI Confluence (ข้อ 6)
# ==========================================
def get_mtf_rsi_alignment(df_4h: pd.DataFrame, df_1d: pd.DataFrame) -> dict:
    """
    ตรวจ RSI หลาย TF ชี้ทางเดียวกัน
    4H Oversold + 1D Oversold = สัญญาณแข็งมาก
    """
    result = {
...
# ==========================================
# [NEW] Signal Scoring System (ข้อ 7)
# ==========================================
def calculate_signal_score(
    ...
    """
    Score 0-100  |  threshold MINIMUM_SIGNAL_SCORE
    ยิ่งสูง = สัญญาณแข็งแกร่ง
    """
    score   = 0
    factors = []
...
# ==========================================
# On-Chain (เฉพาะ coin ที่รองรับ + มีสัญญาณแล้ว)
# ==========================================
...
def scan_market():
    ...
    total_valid_coins                 = 0
    coin_trends_summary               = []
    active_signal_count               = 0   # สำหรับ position sizing

    # ─── Phase 1: Bulk fetch ทั้งหมดก่อน (ลด API calls ~60%) ───
    ...
    # ─── Phase 3: Bulk Funding Rate (1 call แทน N calls) ───
    ...
    # ─── Phase 4: สแกนทุก coin (ไม่มี sleep ใน loop แล้ว) ───
    ...
    for coin in COINS:
        ...
        if df is None or len(df) < EMA_LONG + 10:
            logger.warning(f"{coin}: ข้อมูลไม่พอ – ข้าม")
            continue

        # ─── Context Analysis (ใช้ data ที่ fetch แล้ว) ───
        ...
        candle_info = confirm_reversal_candle(df)          # [NEW]
        mtf_info    = get_mtf_rsi_alignment(df, df_daily)  # [NEW]
        ...
        if signal_type:
            ...
            if sig_score < MINIMUM_SIGNAL_SCORE:
                logger.info(f"{coin}: Score {sig_score} ต่ำกว่า threshold ({MINIMUM_SIGNAL_SCORE}) – ข้ามสัญญาณ")
                signal_type = ""  # กรองออก

        if signal_type:
            # On-Chain: เฉพาะ coin ที่รองรับ + มีสัญญาณแล้ว (ข้อ 5)
            ...
            # ─── [NEW] Correlation-Adjusted Position Sizing ───
            ...
            buy_signals.append({
                ...
                "ob_info":       ob_info,
                "fvg_info":      fvg_info,
                "candle_info":   candle_info,    # [NEW]
                "mtf_info":      mtf_info,       # [NEW]
                "sig_score":     sig_score,      # [NEW]
                "score_label":   score_label,    # [NEW]
                ...
                "dyn_multiplier": dynamic_multiplier,  # [NEW]
                "pos_size":      f"${pos_size:.2f}",
                "sl_risk_pct":   f"{sl_distance_pct*100:.1f}%",
            })

        # Sell signals (overbought / bearish OB)
...
# ==========================================
# Fallback helpers (ถ้าไม่มี 1D data)
# ==========================================
...
def build_messages(buy_list: list, sell_list: list, market_summary: str) -> list:
    ...
    if buy_list:
        ...
        for opt in buy_list:
            ...
            ob   = opt["ob_info"]
            fvg  = opt["fvg_info"]
            ci   = opt["candle_info"]    # [NEW]
            mtfi = opt["mtf_info"]       # [NEW]
...
if __name__ == "__main__":
    logger.info("=" * 60)
    logger.info("Crypto Screener v2 – Starting (Optimized + Signal Score)")
    logger.info("=" * 60)
