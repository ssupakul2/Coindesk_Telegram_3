...
def _record_early_watch_fallback(coin: str, price: float, rsi: float, df, vol_confirmed: bool,
    ...
    """
    [2-Tier System Fix] Fallback สำหรับ Early Watch เมื่อ buy signal ถูกตั้งค่าไว้แล้ว
    (จาก Signal A-F หรือ Fix A/Deep Reversal/Deep Support) แต่ถูก filter chain
    บล็อกทิ้งทีหลัง (Macro/Funding/Volume/1D-Trend/Cooldown/Max-Position/Correlation)
    ...
    ใหม่: เรียก helper นี้ทุกจุดที่ `continue` ใน filter chain — ถ้า RSI อยู่ใน
          ช่วง Early Watch ให้บันทึกไว้ก่อนที่จะข้าม coin นั้นไปจริงๆ
    """
    if not (SIMPLE_OVERSOLD_RSI_THRESHOLD < rsi <= EARLY_WATCH_RSI_THRESHOLD):
        return
