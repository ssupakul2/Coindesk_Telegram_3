...
# [Simple Oversold Signal] RSI threshold สำหรับสัญญาณ "ยึด RSI เป็นหลัก"
# ใช้ค่าเดียวกันทั้งจุดที่ trigger signal และจุดที่ปกป้องจาก mutual exclusion
# เพื่อไม่ให้เกิดช่องว่าง (เช่น signal trigger ที่ 35 แต่ protection ที่ 32)
SIMPLE_OVERSOLD_RSI_THRESHOLD = 35

# [2-Tier System] Early Watch: RSI 35-40 — จับตามอง ไม่ใช่ buy signal เต็มรูปแบบ
# เหตุผล: RSI 36-40 ยังไม่ถือว่า oversold ตามนิยามทั่วไป (classic <30, wide zone <40)
# การ trigger เป็น buy signal เต็มที่ RSI ระดับนี้เสี่ยง false positive สูง
# (อาจ catch falling knife ถ้าตลาด downtrend แรง) จึงแยกเป็น "เฝ้าดู" แทน
EARLY_WATCH_RSI_THRESHOLD = 40
...
def get_volume_trend_info(df: pd.DataFrame) -> dict:
    """
    [Simple Oversold] วิเคราะห์ Volume Trend สำหรับประกอบการตัดสินใจ:
    - current_vs_avg: volume ปัจจุบันเทียบ MA20 (%)
    - trend_direction: volume กำลังเพิ่มขึ้น/ลดลงใน 5 แท่งล่าสุด
    - label: ข้อความสรุปสำหรับ Telegram
    """
    result = {
        "current_vs_avg_pct": None,
        "trend_direction": "unknown",
        "label": "⚪ Volume: ไม่มีข้อมูล",
    }
    if df is None or len(df) < 25 or "volumeto" not in df.columns or "VOL_MA20" not in df.columns:
        return result
    try:
        ...
        vs_avg_pct = ((vol_now - vol_ma) / vol_ma) * 100
        result["current_vs_avg_pct"] = round(vs_avg_pct, 1)

        # Trend: เปรียบเทียบ volume เฉลี่ย 5 แท่งล่าสุด vs 5 แท่งก่อนหน้า
        recent_5 = df["volumeto"].iloc[-5:].mean()
        prev_5   = df["volumeto"].iloc[-10:-5].mean()
        if prev_5 > 0:
        ...
        # Label
        if vs_avg_pct >= 50:
            vol_desc = f"🔊 สูงกว่าเฉลี่ยมาก (+{vs_avg_pct:.0f}%)"
        elif vs_avg_pct >= 0:
            vol_desc = f"🔉 ปกติ ({vs_avg_pct:+.0f}%)"
        else:
            vol_desc = f"🔇 ต่ำกว่าเฉลี่ย ({vs_avg_pct:.0f}%)"

        trend_desc = {
            "increasing": "📈 กำลังเพิ่มขึ้น",
            "decreasing": "📉 กำลังลดลง",
            "stable": "➡️ คงที่",
            "unknown": "",
        }.get(result["trend_direction"], "")

        result["label"] = f"{vol_desc} | เทรนด์ 5 แท่ง: {trend_desc}" if trend_desc else vol_desc
    except Exception as e:
        logger.debug(f"get_volume_trend_info error: {e}")
    return result


def get_entry_timing_advice(rsi: float, bounce_info: dict, candle_info: dict,
                             div_info: dict, mtf_info: dict, vol_trend: dict) -> dict:
    """
    [Simple Oversold] สร้างคำแนะนำ Entry Timing ที่ชัดเจน:
    "รอ Confirm" vs "น่าเข้าซื้อ" ตามจำนวน confirmation signals ที่มี
    เพราะ Simple Oversold ไม่มี key level/uptrend บังคับ จึงต้องบอกชัดว่า
    ควรรอสัญญาณยืนยันเพิ่มเติมหรือเข้าได้เลย
    """
    confirmations = []
    confirm_count = 0

    # 1. Bounce pattern
    if bounce_info.get("quality") in ("strong", "moderate"):
        confirmations.append(f"✅ RSI Bounce: {bounce_info.get('quality')}")
        confirm_count += 1
    elif bounce_info.get("quality") == "weak":
        confirmations.append("🟡 RSI Bounce: อ่อน")
    else:
        confirmations.append("❌ ยังไม่มี RSI Bounce ชัดเจน")

    # 2. Candle reversal pattern
    if candle_info.get("reversal_strength", 0) >= 2:
        confirmations.append(f"✅ Candle ยืนยัน: strength {candle_info['reversal_strength']}")
        confirm_count += 1
    elif candle_info.get("reversal_strength", 0) >= 1:
        confirmations.append("🟡 Candle: สัญญาณอ่อน")
    else:
        confirmations.append("❌ ยังไม่มี Candle ยืนยัน")

    # 3. Divergence
    if div_info and div_info.get("any_bullish"):
        confirmations.append("✅ มี Bullish Divergence")
        confirm_count += 1

    # 4. 1H timing
    if mtf_info.get("is_1h_bouncing"):
        confirmations.append("✅ 1H RSI กำลัง Bounce")
        confirm_count += 1

    # 5. Volume
    if vol_trend.get("trend_direction") == "increasing":
        confirmations.append("✅ Volume กำลังเพิ่มขึ้น")
        confirm_count += 1
    elif vol_trend.get("trend_direction") == "decreasing":
        confirmations.append("🟡 Volume กำลังลดลง — รอดูก่อน")

    # สรุปคำแนะนำ
    if confirm_count >= 3:
        verdict = "🟢 น่าเข้าซื้อ"
        advice = "มี confirmation signals หลายตัวพร้อมกัน — สัญญาณค่อนข้างหนักแน่น"
    elif confirm_count >= 1:
        verdict = "🟡 รอ Confirm เพิ่มเติม"
        advice = "RSI oversold แล้วแต่ confirmation ยังไม่ครบ — แนะนำรอ 1-2 แท่งดู bounce ก่อนเข้าเต็มจำนวน หรือเข้าไม้เล็กนำร่อง"
    else:
        verdict = "🔴 ยังไม่แนะนำเข้า — เฝ้าดูก่อน"
        advice = "RSI ต่ำแต่ยังไม่มีสัญญาณดีดกลับเลย อาจร่วงต่อได้ (catching a falling knife) — รอ bounce/candle ยืนยันก่อน"
...
def calculate_signal_score(rsi, bounce_info, candle_info, vol_confirmed, in_fibo_zone, in_ob_zone,
    ...
    is_simple_oversold_type = "Simple Oversold" in signal_type

    # --- Regime + Signal-type adjusted weights ---
    if is_simple_oversold_type:
        # Simple Oversold: ไม่มี key level/uptrend/bounce ยืนยัน
        # ให้น้ำหนัก RSI หนักที่สุด (เป็น factor หลักเดียวที่ confirm signal นี้)
        # volume และ MTF ยังสำคัญเพื่อกรองคุณภาพ ส่วน level/bounce ไม่เกี่ยวข้อง (=0 อยู่แล้ว)
        w_rsi=1.6; w_bounce=0.6; w_candle=0.6; w_vol=1.4; w_level=0.5; w_div=1.0; w_mtf=1.2
    ...
    # ─────────────────────────────────────────────────────────────
    if is_simple_oversold_type:
        # Simple Oversold: ไม่มี key level/uptrend ยืนยัน ดังนั้น RSI ต้องรับภาระคะแนนหลัก
        # ให้ generous กว่าปกติ เพราะ w_rsi=1.6 (สูงสุด) จะคูณค่านี้อีกที
        # ปรับให้ RSI<=35 (เกณฑ์เข้า signal นี้) + volume ผ่าน MINIMUM_SIGNAL_SCORE ได้แน่นอน
        rsi_base = (32 if rsi <= RSI_OVERSOLD else
                    25 if rsi <= 35 else
                    16 if rsi <= 40 else
                    8  if rsi <= 50 else 0)
    elif market_regime == "bull" and 30 <= rsi <= 50:
...
# Market Scanner
# ==========================================
EXIT_WATCH_RSI_SPLIT_THRESHOLD = 60  # RSI >= นี้ → "RSI Overbought" list, < นี้ → "Trend/Technical Exit" list

def _route_exit_signal(signal_dict: dict, exit_watch_overbought: list,
                        exit_watch_technical: list, exit_watch_signals: list) -> None:
    """
    [New] แยก exit signal เข้า list ตาม RSI เพื่อไม่ให้ oversold/overbought ปนกัน:
    - RSI >= EXIT_WATCH_RSI_SPLIT_THRESHOLD (60) → exit_watch_overbought
      (เหตุผลคือ RSI สูงจริง — ตรงกับ common sense ว่า "RSI สูง = ขาย")
    - RSI < 60 → exit_watch_technical
      (เหตุผลคือ trend break / bearish pattern / ADX อ่อนแรง ฯลฯ ไม่ใช่ RSI)
    exit_watch_signals (combined list) ยังคงได้รับทุก signal เหมือนเดิม
    เพื่อ backward compatibility กับโค้ดส่วนอื่นที่อาจอ้างอิงอยู่ (เช่น portfolio alert)
    """
    rsi_val = signal_dict.get("rsi", 0)
    exit_watch_signals.append(signal_dict)
    if rsi_val >= EXIT_WATCH_RSI_SPLIT_THRESHOLD:
        exit_watch_overbought.append(signal_dict)
    else:
        exit_watch_technical.append(signal_dict)


def _record_early_watch_fallback(coin: str, price: float, rsi: float, df, vol_confirmed: bool,
                                  trend_info: dict, bounce_info: dict,
                                  early_watch_signals: list, block_reason: str) -> None:
    """
    [2-Tier System Fix] Fallback สำหรับ Early Watch ครอบคลุม 2 กรณีที่ coin
    อาจ "ตกหล่น" ไม่มี signal ใดๆ รองรับเลย:

    กรณี 1: RSI 35-40 (Early Watch zone ปกติ)
      buy signal ถูกตั้งค่าไว้แล้ว (จาก Signal A-F หรือ Fix A/Deep Reversal/
      Deep Support) แต่ถูก filter chain บล็อกทิ้งทีหลัง

    กรณี 2: RSI <= 35 (Simple Oversold zone) แต่ vol_confirmed=False
      Simple Oversold signal ต้องการ "rsi<=35 AND vol_confirmed=True" พร้อมกัน
      ถ้า volume ไม่ผ่าน → ไม่เข้า Simple Oversold และไม่เข้า Early Watch ปกติ
      (เพราะ Early Watch zone คือ 35<rsi<=40 ไม่รวม rsi<=35) → ตกหล่นไม่มีที่ไป
      จึงต้องรับเคสนี้ไว้ด้วยเป็น Early Watch เช่นกัน แต่ระบุเหตุผลต่างจากกรณี 1

    เดิม: ถ้า signal_type ถูกตั้งแล้วและ RSI อยู่ในช่วง 35-40 ก่อนจะถูก filter บล็อก
          coin นั้นจะหายไปเงียบๆ ไม่กลับมาเป็น Early Watch เลย (เพราะ Early Watch
          check เดิมอยู่ "ก่อน" filter chain — เห็นว่า signal_type ไม่ว่างก็ข้ามไปแล้ว)
    ใหม่: เรียก helper นี้ทุกจุดที่ `continue` ใน filter chain — ถ้า RSI อยู่ใน
          ช่วงที่เกี่ยวข้อง ให้บันทึกไว้ก่อนที่จะข้าม coin นั้นไปจริงๆ
    """
    in_early_watch_zone = SIMPLE_OVERSOLD_RSI_THRESHOLD < rsi <= EARLY_WATCH_RSI_THRESHOLD
    in_oversold_no_vol   = rsi <= SIMPLE_OVERSOLD_RSI_THRESHOLD and not vol_confirmed

    if not (in_early_watch_zone or in_oversold_no_vol):
        return

    vol_trend_ew = get_volume_trend_info(df)

    if in_oversold_no_vol:
        note = (
            f"RSI {rsi:.1f} อยู่ในโซน Oversold (≤{SIMPLE_OVERSOLD_RSI_THRESHOLD}) "
            f"แต่ Volume ไม่ยืนยัน (Simple Oversold ต้องการทั้ง RSI ต่ำ + Volume ยืนยันพร้อมกัน) "
            f"— สัญญาณซื้อเดิมถูกระงับ ({block_reason}) จับตาดูไว้ก่อน "
            f"รอ Volume กลับมายืนยันหรือ RSI ลงต่อ"
        )
    else:
        note = (
            f"RSI {rsi:.1f} เริ่มอ่อนแรง (โซน {SIMPLE_OVERSOLD_RSI_THRESHOLD}-{EARLY_WATCH_RSI_THRESHOLD}) "
            f"แต่ยังไม่ถึงเกณฑ์ Oversold เต็มรูปแบบ (≤{SIMPLE_OVERSOLD_RSI_THRESHOLD}) "
            f"— สัญญาณซื้อเดิมถูกระงับ ({block_reason}) จับตาดูไว้ก่อน "
            f"รอ RSI ลงต่อหรือมีสัญญาณ bounce ชัดเจนกว่านี้"
        )

    early_watch_signals.append({
        "coin": coin, "price": format_price(price), "rsi": round(rsi, 2),
        "vol_confirmed": vol_confirmed,
        "vol_trend_label": vol_trend_ew["label"],
        "trend_label": trend_info["trend_label"],
        "bounce_label": bounce_info["quality_label"],
        "note": note,
    })
    logger.info(f"🔍 {coin}: ย้ายไป Early Watch แทน (signal ถูกบล็อก: {block_reason}, RSI:{rsi:.1f})")
...
def scan_market(positions: dict):
    ...
    early_watch_signals = []  # [2-Tier] RSI 35-40 — จับตามอง ไม่ใช่ buy signal เต็มรูปแบบ
    # [New] แยก Exit Watch เป็น 2 กลุ่มตาม RSI เพื่อไม่ให้ปนกันจนสับสน:
    #   exit_watch_overbought: RSI >= 60 — เหตุผลคือ RSI สูง (ตรง common sense)
    #   exit_watch_technical:  RSI < 60  — เหตุผลคือ trend/pattern อื่น (ไม่ใช่ RSI)
    exit_watch_overbought = []
    exit_watch_technical  = []
    ...
    for coin in COINS:
        ...
        bias_warning = ""
        # [2-Tier System] flag บอกว่า coin นี้เข้าเกณฑ์ RSI 35-40 หรือไม่
        # ใช้เป็น fallback ท้ายสุด ถ้า signal_type ที่ตั้งไว้ถูก filter บล็อกทิ้ง
        _early_watch_candidate = SIMPLE_OVERSOLD_RSI_THRESHOLD < rsi <= EARLY_WATCH_RSI_THRESHOLD
        ...
        if price > ema200:
            ...
            # ─────────────────────────────────────────────────────────────
            # Signal F: Simple Oversold (ใหม่)
            # จับเคส "RSI ต่ำธรรมดา" ที่ไม่เข้าเงื่อนไข Signal A-E เลย
            # เพราะ Signal A-E ทุกตัวต้องการบริบทเพิ่มเติม (key level / uptrend
            # structure / bounce pattern ที่ชัดเจน) ทำให้พลาดเคสจำนวนมากที่
            # RSI oversold จริงแต่ไม่ได้อยู่ตรง pattern ที่กำหนดไว้พอดี
            #
            # เงื่อนไขขั้นต่ำที่สุด: แค่ RSI oversold + volume ยืนยัน
            # ไม่ต้องการ key level, ไม่ต้องการ uptrend, ไม่ต้องการ bounce confirm
            # → เหมาะกับ "เริ่มจับตา/ทยอยช้อนซื้อ" มากกว่า "high-confidence entry"
            # signal score จะคัดกรองคุณภาพอีกชั้นหนึ่งตามปกติ
            # ─────────────────────────────────────────────────────────────
            if (not signal_type
                    and rsi <= SIMPLE_OVERSOLD_RSI_THRESHOLD
                    and vol_confirmed):
                signal_type = "Simple Oversold 🟢"
                signal_reason = f"RSI oversold เรียบง่าย {rsi:.1f}, volume ยืนยัน"
            ...
            if est_target is not None and price >= est_target and coin in positions:
                ...
                exit_watch_technical.append(tp_alert)  # TP target = positive event, ไม่ใช่ RSI overbought warning
            ...
            if exit_score >= effective_exit_threshold:
                ...
                # [RSI-First Priority] ถ้า signal_type คือ Simple Oversold และ RSI
                # ยังอยู่ในโซน oversold ตามเกณฑ์เดียวกับที่ trigger signal นี้
                # (SIMPLE_OVERSOLD_RSI_THRESHOLD) ให้ Buy ชนะเด็ดขาด
                # ตามหลักการ "ยึด RSI เป็นหลัก" — ไม่ route เข้า Exit Watch เลย
                # เพราะ exit_score มาจาก factor อื่น (bearish candle/ADX/trend) ที่ไม่ใช่ RSI
                # ซึ่งขัดกับเกณฑ์หลักของสัญญาณนี้โดยตรง
                # ใช้ threshold เดียวกับจุด trigger signal เพื่อไม่ให้เกิดช่องว่าง
                # (เช่น RSI=35 trigger signal ได้ แต่ protection คุ้มครองแค่ <=32)
                is_protected_simple_oversold = (
                    "Simple Oversold" in signal_type and rsi <= SIMPLE_OVERSOLD_RSI_THRESHOLD
                )

                if is_protected_simple_oversold:
                    logger.info(
                        f"🛡️ {coin}: ระงับ Exit Watch (exit_score:{exit_score}) เพราะ Simple Oversold "
                        f"ชนะตามหลักการ RSI-first (RSI:{rsi:.1f} <= {SIMPLE_OVERSOLD_RSI_THRESHOLD}) — คงเป็น Buy Signal"
                    )
                    # ไม่ route เข้า exit watch — ปล่อยให้ signal_type ผ่านไปเป็น Buy Signal ปกติ
                else:
                    _route_exit_signal(exit_signal_dict, exit_watch_overbought, exit_watch_technical, exit_watch_signals)
            ...
            if coin in positions:
                ...
                if not skip_exit_watch_for_coin and (exit_score_b >= eff_threshold_b or force_include):
                    ...
                    _route_exit_signal(exit_signal_dict_b, exit_watch_overbought, exit_watch_technical, exit_watch_signals)
        ...
        if signal_type and coin not in positions:
            ...
            if macro_info.get("block_new_entries"):
                ...
                _record_early_watch_fallback(coin, price, rsi, df, vol_confirmed, trend_info, bounce_info, early_watch_signals, "Macro Event")
                continue
            ...
            if funding_info.get("block_long"):
                ...
                if is_momentum_signal and fr < FUNDING_RATE_MAX_LONG * 2:
                    ...
                    _record_early_watch_fallback(coin, price, rsi, df, vol_confirmed, trend_info, bounce_info, early_watch_signals, "Funding Rate สูง")
                    continue
            ...
            if not volume_info.get("passes", True):
                ...
                _record_early_watch_fallback(coin, price, rsi, df, vol_confirmed, trend_info, bounce_info, early_watch_signals, "Volume ต่ำ")
                continue
            ...
            if not trend_1d.get("aligned", True):
                if is_extreme_oversold_exception:
                    ...
                    _record_early_watch_fallback(coin, price, rsi, df, vol_confirmed, trend_info, bounce_info, early_watch_signals, "1D Trend ไม่ aligned")
                    continue
            ...
            if in_cd:
                ...
                _record_early_watch_fallback(coin, price, rsi, df, vol_confirmed, trend_info, bounce_info, early_watch_signals, "Re-entry Cooldown")
                continue
            ...
            if len(positions) >= MAX_CONCURRENT_POSITIONS:
                ...
                _record_early_watch_fallback(coin, price, rsi, df, vol_confirmed, trend_info, bounce_info, early_watch_signals, "Max Concurrent Positions")
                continue
            ...
            if corr_btc >= HIGH_CORR_BTC_THRESHOLD and existing_high_corr_count >= MAX_HIGH_CORR_POSITIONS:
                ...
                _record_early_watch_fallback(coin, price, rsi, df, vol_confirmed, trend_info, bounce_info, early_watch_signals, "Correlation Cap")
                continue
            ...
            if sig_score >= MINIMUM_SIGNAL_SCORE:
                ...
                # [Simple Oversold] ข้อมูลเสริมสำหรับสัญญาณนี้โดยเฉพาะ
                # เพราะไม่มี key level/uptrend บังคับ ต้องการ context เพิ่มเติม
                # เพื่อช่วยตัดสินใจว่า "รอ confirm" หรือ "น่าเข้าซื้อ"
                is_simple_oversold_signal = "Simple Oversold" in signal_type
                simple_oversold_extra = {}
                if is_simple_oversold_signal:
                    vol_trend_info = get_volume_trend_info(df)
                    timing_advice  = get_entry_timing_advice(
                        rsi, bounce_info, candle_info, div_info, mtf_info, vol_trend_info
                    )
                    ob_label = (
                        f"{ob_info['bullish_ob_label']} Order Block: "
                        f"${format_price(ob_info['bullish_ob_price'])} - ${format_price(ob_info['bullish_ob_top'])}"
                        if ob_info.get("has_bullish_ob")
                        else "⚪ ไม่พบ Order Block ใกล้ราคาปัจจุบัน"
                    )
                    simple_oversold_extra = {
                        "vol_trend_label": vol_trend_info["label"],
                        "ob_label": ob_label,
                        "entry_verdict": timing_advice["verdict"],
                        "entry_advice": timing_advice["advice"],
                        "entry_confirmations": timing_advice["confirmations"],
                        "entry_confirm_count": timing_advice["confirm_count"],
                    }
                ...
                buy_signals.append({
                    ...
                    "is_simple_oversold": is_simple_oversold_signal,
                    **simple_oversold_extra,
                    "partial_tp_note": (
        ...
        elif coin not in positions:
            # [2-Tier System Fallback] coin ที่ไม่เคยมี signal_type ตั้งแต่แรกเลย
            # (ไม่ผ่าน Signal A-F หรือ Fix A/Deep Reversal/Deep Support ใดๆ)
            # ตรวจ Early Watch candidate ตรงนี้ — ครอบคลุม normal case ส่วนใหญ่
            # ของ Early Watch (ไม่ได้ถูก filter บล็อก แค่ไม่เคยมี signal ตั้งแต่ต้น)
            _record_early_watch_fallback(coin, price, rsi, df, vol_confirmed, trend_info,
                                          bounce_info, early_watch_signals, "ไม่มีสัญญาณซื้อในรอบนี้")
    ...
    return (buy_signals, exit_watch_signals, position_updates, summary_msg, supply_zone_alerts,
            watch_reversal_signals, exit_watch_overbought, exit_watch_technical, early_watch_signals)

def build_messages(buy_list, exit_watch_list, position_updates, market_summary,
                    supply_zone_alerts=None, watch_reversal_list=None,
                    exit_watch_overbought=None, exit_watch_technical=None,
                    early_watch_list=None) -> list:
    ...
    if buy_list:
        ...
        for opt in buy_list:
            ...
            if opt.get("is_simple_oversold"):
                # [Simple Oversold] Template เฉพาะ — เน้น RSI เป็นหลัก พร้อมข้อมูล
                # ประกอบการตัดสินใจครบ: volume trend, candle, OB, TP targets, entry advice
                confirmations_txt = "\n".join(f"  {c}" for c in opt.get("entry_confirmations", []))
                msg = (
                    f"\n\n🟢 <b>{opt['coin']}</b> | ราคา: ${opt['price']}\n"
                    f"🚨 สัญญาณ: <b>{opt['type']}</b> | <b>RSI 4H: {opt['rsi']}</b> (เกณฑ์หลัก)\n"
                    f"🏆 {opt['score_label']}\n\n"
                    f"📊 <b>ข้อมูลประกอบการตัดสินใจ:</b>\n"
                    f"🔊 Volume: {opt.get('vol_trend_label', 'N/A')}\n"
                    f"{opt['reversal_label']}\n"
                    f"🔄 RSI Bounce: {opt['bounce_label']}\n"
                    f"📦 {opt.get('ob_label', '')}\n"
                    f"📡 MTF: {opt['mtf_label']}\n\n"
                    f"🎯 <b>เป้าหมายราคา:</b>\n"
                    f"🚀 TP1: <code>{opt['tp1']}</code> | TP1.5: <code>{opt['tp1_5']}</code> | "
                    f"TP2: <code>{opt['tp2']}</code>{tp_method_note}\n"
                    f"🔥 Dynamic TP (RSI70 est.): <code>{opt['dynamic_tp']}</code>\n"
                    f"❌ Hard SL: <code>{opt['sl']}</code>\n\n"
                    f"<b>{opt.get('entry_verdict', '')}</b>\n"
                    f"📝 {opt.get('entry_advice', '')}\n"
                    f"Confirmation Checklist ({opt.get('entry_confirm_count', 0)}/5):\n"
                    f"{confirmations_txt}\n\n"
    ...
    # --- [2-Tier System] Early Watch Section (RSI 35-40, จับตามอง) ---
    if early_watch_list:
        current = (
            "🔍 <b>[Early Watch – จับตามอง RSI เริ่มอ่อนแรง]</b>\n"
            f"<i>RSI {SIMPLE_OVERSOLD_RSI_THRESHOLD}-{EARLY_WATCH_RSI_THRESHOLD} — ยังไม่ใช่ Buy Signal "
            "แต่เริ่มน่าสนใจ รอ RSI ลงต่อหรือ bounce ชัดเจนก่อนตัดสินใจ</i>"
        )
        for opt in early_watch_list:
            vol = "🔊 Volume ยืนยัน" if opt.get("vol_confirmed") else "🔇 Volume ต่ำ"
            msg = (
                f"\n\n🔵 <b>{opt['coin']}</b> | ราคา: ${opt['price']} | RSI: {opt['rsi']}\n"
                f"{vol} | {opt.get('vol_trend_label', '')}\n"
    ...
    # --- Exit Watch Sections (แยกตาม RSI เพื่อไม่ให้ oversold/overbought ปนกัน) ---
    def _render_exit_watch_block(items: list, header: str, header_emoji: str) -> None:
        if not items:
            return
        current = header
        for opt in sorted(items, key=lambda x: -x["exit_score"]):
            ...
            if len(current) + len(msg) > 3500: blocks.append(current); current = f"{header_emoji} <b>[ต่อ]</b>" + msg
            else: current += msg
        blocks.append(current)

    # ส่วนที่ 1: RSI Overbought (RSI >= 60) — เหตุผลคือ RSI สูงจริง ตรง common sense
    _render_exit_watch_block(
        exit_watch_overbought or [],
        "🔴 <b>[Exit Watch – RSI Overbought]</b>\n"
        f"<i>RSI ≥ {EXIT_WATCH_RSI_SPLIT_THRESHOLD} — เหตุผลคือ RSI สูงจริง เรียงตาม Exit Score สูงสุดก่อน</i>",
        "🔴",
    )

    # ส่วนที่ 2: Trend/Technical Exit (RSI < 60) — เหตุผลอื่นที่ไม่ใช่ RSI สูง
    _render_exit_watch_block(
        exit_watch_technical or [],
        "🟠 <b>[Exit Watch – Trend/Technical Exit]</b>\n"
        f"<i>RSI &lt; {EXIT_WATCH_RSI_SPLIT_THRESHOLD} — เหตุผลคือ trend break/pattern อื่น "
        "(ไม่ใช่ RSI สูง) เรียงตาม Exit Score สูงสุดก่อน</i>",
        "🟠",
    )
...
if __name__ == "__main__":
    ...
    (buy_list, exit_watch_list, position_updates, market_summary, supply_zone_alerts,
     watch_reversal_list, exit_watch_overbought, exit_watch_technical, early_watch_list) = scan_market(positions)
    send_telegram_messages(build_messages(
        buy_list, exit_watch_list, position_updates, market_summary, supply_zone_alerts,
        watch_reversal_list, exit_watch_overbought, exit_watch_technical, early_watch_list,
    ))
