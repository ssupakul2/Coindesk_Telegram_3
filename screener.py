...
# หมายเหตุ: ย้ายไปพึ่งพา Binance API เป็นหลักแล้ว 
# ตัวแปรนี้จะใช้เป็น Safe Fallback เผื่อดึง On-Chain ข้อมูลวาฬบางส่วนเท่านั้น (หากพัง ระบบจะข้ามให้อัตโนมัติ บอทไม่หยุดทำงาน)
...
# ==========================================
API_RATE_LIMIT_DELAY = 0.5   # หน่วงเวลาปลอดภัยสำหรับ Binance Public API
API_MAX_RETRIES      = 3
...
api_session.headers.update({
    "User-Agent": "CryptoScreenerBot/3.0 (Binance Anti-451 Endpoint)",
...
# ==========================================
# Data Fetching (แก้ไขเป็น ท่อสาธารณะ ป้องกัน 451)
# ==========================================
def get_historical_data(coin: str) -> pd.DataFrame | None:
    """ดึงข้อมูลแท่งเทียน 4H จาก Binance Public Data (แก้ทางปัญหา Geo-blocking 451 บน GitHub)"""
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            logger.warning(f"{coin} 4H Binance Error Status: {resp.status_code}")
    except Exception as e:
        logger.warning(f"{coin} 4H Binance Fetch Exception: {e}")
    return None


def get_histoday_data(coin: str) -> pd.DataFrame | None:
    """ดึงข้อมูลแท่งเทียน 1D จาก Binance Public Data (แก้ทางปัญหา Geo-blocking 451 บน GitHub)"""
    url = "https://data.binance.com/api/v3/klines"
    symbol = f"{coin}USDT"
    params = {
        "symbol": symbol,
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            logger.warning(f"{coin} 1D Binance Error Status: {resp.status_code}")
    except Exception as e:
        logger.warning(f"{coin} 1D Binance Fetch Exception: {e}")
    return None
...
def scan_market():
    ...
    logger.info("Phase 1: Bulk fetch 4H data from Binance Public Endpoint...")
    all_4h = bulk_fetch_4h(COINS)

    logger.info("Phase 2: Bulk fetch 1D data from Binance Public Endpoint...")
    ...
    if total_valid_coins > 0:
        ...
        summary_msg   = (
            f"📊 <b>[Market Summary – 🤖 Binance Platform Engine]</b>\n"
...
if __name__ == "__main__":
    logger.info("=" * 60)
    logger.info("Crypto Screener v3.0 – Active System Engine Running (Binance Pipeline)")
    logger.info("=" * 60)
    ...
    logger.info("ระบบรันเสร็จสิ้นผ่าน Binance Endpoint ไร้ปัญหา 451 บล็อก!")
