...
def scan_market(positions: dict):
    ...
    for coin in COINS:
        ...
        if price > ema200:
            ...
            # ─────────────────────────────────────────────────────────────
            # [2-Tier System] Early Watch — RSI 35-40 (ไม่ใช่ buy signal เต็มรูปแบบ)
            # จับเคสที่ RSI เริ่มอ่อนแรงแต่ยังไม่ถึงเกณฑ์ oversold เต็มที่
            # ไม่ trigger signal_type (ไม่กระทบ buy signal logic เดิมเลย)
            # แต่บันทึกแยกไว้ใน early_watch_signals เพื่อ "จับตามอง"
            # ─────────────────────────────────────────────────────────────
            if (not signal_type
                    and SIMPLE_OVERSOLD_RSI_THRESHOLD < rsi <= EARLY_WATCH_RSI_THRESHOLD):
                ...
                early_watch_signals.append({
                    ...
                    "note": (
                        ...
                        f"— จับตาดูไว้ก่อน รอ RSI ลงต่อหรือมีสัญญาณ bounce ชัดเจนกว่านี้"
                    ),
                })
            ...
            # ─────────────────────────────────────────────────────────────
            # [2-Tier System] Early Watch — RSI 35-40 ใน bearish branch (price < EMA200)
            # เคสนี้ครอบคลุม coin ที่ราคาต่ำกว่า EMA200 ด้วย เพราะ RSI 35-40
            # ไม่ควรถูกมองข้ามแค่เพราะ trend รายใหญ่ยังเป็นขาลง — ยังคุ้มค่าจับตามอง
            # ใช้เงื่อนไขเดียวกับฝั่ง bullish branch แต่ไม่ต้องการ vol_confirmed
            # เพราะเป็นแค่ "จับตามอง" ไม่ใช่ buy signal เต็มรูปแบบ
            # ไม่ trigger สำหรับ coin ที่มี position อยู่แล้ว (Fix H ดูแลแยกต่างหาก)
            # ─────────────────────────────────────────────────────────────
            if (not signal_type
                    and coin not in positions
                    and SIMPLE_OVERSOLD_RSI_THRESHOLD < rsi <= EARLY_WATCH_RSI_THRESHOLD):
                vol_trend_ew = get_volume_trend_info(df)
                early_watch_signals.append({
                    "coin": coin, "price": format_price(price), "rsi": round(rsi, 2),
                    ...
                    "note": (
                        f"RSI {rsi:.1f} เริ่มอ่อนแรง (โซน {SIMPLE_OVERSOLD_RSI_THRESHOLD}-{EARLY_WATCH_RSI_THRESHOLD}) "
                        f"แต่ยังไม่ถึงเกณฑ์ Oversold เต็มรูปแบบ (≤{SIMPLE_OVERSOLD_RSI_THRESHOLD}) "
                        f"— ราคายังต่ำกว่า EMA200 ({bias_warning.replace('⚠️ ', '').replace('🔻 ', '')}) "
                        f"จับตาดูไว้ก่อน รอ RSI ลงต่อหรือมีสัญญาณ bounce ชัดเจนกว่านี้"
                    ),
                })
