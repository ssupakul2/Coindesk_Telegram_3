...
def scan_market(positions: dict):
    ...
    for coin in COINS:
        ...
        # Unique price ratio: ถ้า candle ล่าสุด 20 แท่งมีราคาซ้ำกันมากกว่า 80% = frozen
        # ER = 1.0 + Consistency ต่ำมาก = ราคาไม่ขยับ
        is_frozen = False
        if rsi > 95:
            recent_20 = df["close"].iloc[-20:]
            n_unique = recent_20.nunique()
            unique_ratio = n_unique / len(recent_20)
            cv_scan = recent_20.std() / recent_20.mean() if recent_20.mean() != 0 else 0; n_uniq_scan = recent_20.nunique()
            if cv_scan < 0.0002 and n_uniq_scan < max(len(recent_20)//5, 4):
                is_frozen = True
