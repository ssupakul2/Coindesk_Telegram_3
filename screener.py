            ...
            if coin in positions:
                ...
                if is_below_ema200_closed and is_below_ema200_live:
                    if is_near_ema200_line:
                        ...
                        # หลุดชัดเจน ห่างเกิน 0.75x ATR — severity เต็ม
                ...
                # force-include เฉพาะเมื่อหลุดชัดเจน (ไม่ใช่แค่ near-line)
                # near-line cases ยังคงต้องผ่าน exit_score threshold ปกติ
                force_include = is_below_ema200_live and not is_near_ema200_line
                if exit_score_b >= eff_threshold_b or force_include:
...
def build_messages(buy_list, exit_watch_list, position_updates, market_summary, supply_zone_alerts=None) -> list:
    ...
    if exit_watch_list:
        ...
        for opt in sorted(exit_watch_list, key=lambda x: -x["exit_score"]):
            ...
            rsi_header = ""
            if opt.get("is_near_ema200_line"):
