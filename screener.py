...
# [A] เพิ่มการรองรับ Proxy (หากใช้ .ovpn ระดับ OS อยู่แล้ว ปล่อยว่างไว้ได้เลย)
# รูปแบบตัวอย่าง: "http://user:pass@123.45.67.89:8080" หรือ "socks5://127.0.0.1:1080"
...
# [B] Binance Endpoints (หลัก + .us + mirror)
BINANCE_ENDPOINTS = [
    "https://api.binance.com",          # หลัก
    "https://api3.binance.com",         # cluster
    "https://api1.binance.com",
    "https://api2.binance.com",
    "https://api.binance.us",           # ไม่ถูก geo-block
...
api_session.headers.update({
    "User-Agent": "CryptoScreenerBot/3.8 (A+B+C Multi-Source Engine)",
...
def _fetch_4h_from_coingecko(coin: str) -> pd.DataFrame | None:
    """[C] Fallback 4H จาก CoinGecko OHLC"""
    ...
    try:
        ...
        df["time"] = pd.to_datetime(df["time"], unit="ms")
        df.set_index("time", inplace=True)
        df["volumeto"] = 0.0  # CoinGecko OHLC ไม่มี volume ให้เซ็ตเป็น 0.0
...
# ==========================================
# Core Indicators & Analysis (คงเดิม)
# ==========================================
...
def calculate_signal_score(rsi, bounce_info, candle_info, vol_confirmed, in_fibo_zone, in_ob_zone, in_fvg_zone, weekly_ctx, mtf_info, adx, is_divergence, trend_info) -> tuple[int, str]:
...
def scan_market():
    ...
    for coin in COINS:
        ...
        if signal_type:
            sig_score, score_label = calculate_signal_score(rsi, bounce_info, candle_info, vol_confirmed, in_fibo, in_ob, in_fvg, weekly_ctx, mtf_info, adx, is_div, trend_info)
            if sig_score >= MINIMUM_SIGNAL_SCORE:
                ...
                pos_size = get_correlation_adjusted_position(PORTFOLIO_USDT, RISK_PER_TRADE_PCT, sl_dist, 0.5, active_signal_count)
