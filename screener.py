...
# หมายเหตุ: ถ้าย้ายไปใช้ Binance แล้ว ตัวแปร CRYPTOCOMPARE_API_KEY จะเก็บไว้แค่ใช้ฟังก์ชัน On-Chain (ถ้าคีย์ยังใช้ได้) 
# แต่ถ้าคีย์ขิตไปแล้ว ระบบจะข้าม On-Chain ให้โดยไม่ทำให้บอทพังครับ
...
# ==========================================
API_RATE_LIMIT_DELAY = 0.5   # ปลอดภัยมากสำหรับ Binance Public API
API_MAX_RETRIES      = 3
BINANCE_LIMIT        = 500   # ดึงย้อนหลัง 500 แท่ง เพียงพอต่อ EMA200 
...
# --- On-Chain: ย้ายไปสแตนด์บายเผื่อใช้คีย์เก่า ---
...
api_session.headers.update({
    "User-Agent": "CryptoScreenerBot/2.0 (Binance Dynamic Version)",
...
# ==========================================
# Data Fetching (เปลี่ยนเป็น Binance API - ฟรี ไม่ต้องใช้ Key)
# ==========================================
def get_historical_data(coin: str) -> pd.DataFrame | None:
    """ดึง 4H OHLCV จาก Binance API"""
    ...
    try:
        ...
        if resp.status_code == 200:
            data = resp.json()
            # โครงสร้างผลลัพธ์ Binance: [OpenTime, Open, High, Low, Close, Volume, CloseTime, ...]
        ...
        logger.warning(f"{coin} 4H Binance fetch error: {e}")
    return None


def get_histoday_data(coin: str) -> pd.DataFrame | None:
    """ดึง 1D OHLCV จาก Binance API"""
    url = "https://api.binance.com/api/v3/klines"
    symbol = f"{coin}USDT"
    params = {
        "symbol": symbol,
    ...
    try:
        ...
        logger.warning(f"{coin} 1D Binance fetch error: {e}")
    return None
...
def calculate_signal_score(
    ...
    if score >= 70: grade = "🔥 A+ (แนะนำเข้า)"
    elif score >= 55: grade = "✅ A  (พิจารณาได้)"
    elif score >= MINIMUM_SIGNAL_SCORE: grade = "🟡 B  (รอยืนยันเพิ่ม)"
    else: grade = "⬜ C  (ยังไม่แนะนำ)"
...
def analyze_weekly_context(df_1d: pd.DataFrame) -> dict:
    ...
    try:
        ...
        if result["weekly_bullish_div"]:
            ...
            result["weekly_status_label"] = f"🔥 <b>ภาพใหญ่ Oversold รุนแรง ({curr_rsi_w})</b>"
...
def analyze_cycle_targets(df_1d: pd.DataFrame) -> dict:
    ...
    try:
        ...
        w_ema50  = df_w["EMA_50"].iloc[-1] if len(df_w) >= 50 else curr_price
        w_ema200 = df_w["EMA_200"].iloc[-1] if len(df_w) >= 200 else curr_price
...
# ==========================================
# On-Chain (Safe Fallback Mode เผื่อ API คีย์หมดโควตา)
# ==========================================
...
# ==========================================
# Futures Context (Binance Public Data - ฟรี)
# ==========================================
...
# ==========================================
# Market Scanner Pipeline (ย้ายค่ายมา Binance)
# ==========================================
def scan_market():
    ...
    logger.info("Phase 1: Bulk fetch 4H data from Binance...")
    all_4h = bulk_fetch_4h(COINS)

    logger.info("Phase 2: Bulk fetch 1D data from Binance...")
    ...
    logger.info("Phase 4: Scan all coins...")
    for coin in COINS:
        df        = all_4h.get(coin)
    ...
    if total_valid_coins > 0:
        ...
        summary_msg   = (
            f"📊 <b>[Market Summary Binance v2.5]</b>\n"
            f"ดัชนีหลัก (BTC): <b>{market_regime}</b>\n"
            f"ทุนระบบ: {PORTFOLIO_USDT} USDT | Risk {RISK_PER_TRADE_PCT}%/ไม้\n"
            f"📈 ขาขึ้น: {bullish_coins} | 📉 ขาลง: {bearish_coins} เหรียญ\n"
        )
        if bullish_ratio >= 65: summary_msg += "🔥 ภาพรวม: <b>🟢 Strong Bullish</b>"
        elif bullish_ratio >= 40: summary_msg += "🔥 ภาพรวม: <b>🟡 Sideways</b>"
        else: summary_msg += "🔥 ภาพรวม: <b>🔴 Bearish</b>"
...
def build_messages(buy_list: list, sell_list: list, market_summary: str) -> list:
    ...
    if buy_list:
        header = "🎯 <b>[Crypto Screener Binance 4H – สัญญาณซื้อ]</b>"
        current = header
        for opt in buy_list:
            vol_note = "\n🔊 Volume: <b>ยืนยัน (สูงกว่า MA20)</b>" if opt["vol_confirmed"] else "\n🔇 Volume: ไม่ยืนยัน"
            corr_alert = f"\n⚠️ <b>Correlation BTC:</b> {opt['corr_btc']}" if opt.get("corr_btc", 0) > 0.85 and opt["coin"] != "BTC" else ""
            time_alert = f"\n⏳ <b>ADX ต่ำ ({opt.get('adx',0)}):</b> ตลาด Sideways" if opt.get("squeeze_warning") else ""
            ...
            if fvg.get("has_fvg_support"):
                confluence += f"\n   ⚡ FVG (4H): <code>${format_price(fvg['fvg_bottom'])} - ${format_price(fvg['fvg_top'])}</code>"
            if ob.get("has_bullish_ob"):
                confluence += f"\n   🐳 OB Support: <code>${format_price(ob['bullish_ob_price'])}</code>"

            coin_msg = (
                f"\n\n🪙 <b>{opt['coin']}</b> | ราคา: ${opt['price']}"
                f"\n🚨 สัญญาณ: <b>{opt['type']}</b> | RSI: {opt['rsi']}"
                f"{vol_note}{corr_alert}{time_alert}\n🏆 {opt['score_label']}"
                f"\n📡 MTF: {mtfi.get('mtf_label','')}\n🕯️ Candle: {ci['reversal_label']}"
                f"{confluence}\n📐 เทรนด์: {ti['trend_label']}\n🔄 Bounce: {bi['quality_label']}"
                f"\n🗓️ ภาพใหญ่: {w_ctx.get('weekly_status_label','')}\n📈 {ft.get('futures_label','')}{dc.get('death_cross_label','')}"
                f"\n\n🛡️ <b>[Risk Management]</b>"
                f"\n💼 Position Size: <code>{opt['pos_size']}</code> | ระยะ SL: {opt['sl_risk_pct']}"
                f"\n❌ Hard SL: <code>{opt['sl']}</code>"
                f"\n🚀 <b>[Take Profit Tiers]</b>"
                f"\nTP1: <code>{opt['tp1']}</code> | TP2: <code>{opt['tp2']}</code>"
...
if __name__ == "__main__":
    logger.info("=" * 60)
    logger.info("Crypto Screener v2.5 – Starting (Binance API Engine)")
    logger.info("=" * 60)
    ...
    logger.info("เรียบร้อย! ทำงานผ่าน Binance API สมบูรณ์")
