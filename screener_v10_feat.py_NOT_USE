...
# [Staleness Guard] ถ้า candle ล่าสุดใน df เก่ากว่านี้ (ชั่วโมง) → ถือว่าข้อมูลล้าสมัย
# สำหรับ CoinGecko OHLC ที่ pre-compute bucket ล่วงหน้า candle อาจล้าสมัยได้ถึง 4-12h
DATA_STALENESS_WARN_HOURS  = 5.0   # เตือนถ้า candle ล่าสุดเก่ากว่า 5 ชั่วโมง
DATA_STALENESS_SKIP_HOURS  = 10.0  # ข้าม coin ถ้า candle เก่ากว่า 10 ชั่วโมง (ข้อมูลไม่น่าเชื่อถือ)
...
# [New] Trading Fee
# ------------------------------------------
TRADING_FEE_PCT = 0.001  # 0.1% per side (Binance maker/taker default)

# ------------------------------------------
# [New] Adaptive Position Sizing
# ------------------------------------------
# Position size ปรับตาม signal score:
# BASE_RISK × (score / SCORE_REFERENCE) clamp ระหว่าง MIN_RISK และ MAX_RISK
ADAPTIVE_SIZING_ENABLED  = True
ADAPTIVE_SCORE_REFERENCE = 75.0   # score ที่ถือว่า "full size"
ADAPTIVE_MIN_RISK_PCT    = 1.0    # ขั้นต่ำ 1% ของ portfolio
ADAPTIVE_MAX_RISK_PCT    = 3.0    # สูงสุด 3% ของ portfolio

# ------------------------------------------
# [New] Drawdown Circuit Breaker
# ------------------------------------------
CIRCUIT_BREAKER_ENABLED       = True
CIRCUIT_BREAKER_MAX_LOSSES    = 3      # consecutive SL hits ก่อน pause
CIRCUIT_BREAKER_PAUSE_HOURS   = 24    # pause กี่ชั่วโมง
CIRCUIT_BREAKER_FILE          = os.getenv("CIRCUIT_BREAKER_FILE", "circuit_breaker.json")

# ------------------------------------------
# [New] Fear & Greed Index
# ------------------------------------------
FEAR_GREED_ENABLED        = True
FEAR_GREED_EXTREME_FEAR   = 25    # index <= 25 → bonus +score
FEAR_GREED_EXTREME_GREED  = 75    # index >= 75 → penalty -score
FEAR_GREED_SCORE_BONUS    = 6     # pts บวกเมื่อ Extreme Fear
FEAR_GREED_SCORE_PENALTY  = 6     # pts หักเมื่อ Extreme Greed

# ------------------------------------------
# [New] BTC Dominance — Absolute Threshold
# ------------------------------------------
# ถ้า BTC.D สูงเกิน threshold นี้ = altcoin season ยังไม่มา
# block all alt longs (ยกเว้น BTC และ ETH)
BTCD_ABSOLUTE_BLOCK_THRESHOLD  = 57.0  # BTC.D > 57% → block altcoins
BTCD_ABSOLUTE_CAUTION_THRESHOLD = 54.0 # BTC.D > 54% → -5 signal score

# ------------------------------------------
# [New] Regime-Aware Signal Blacklist
# ------------------------------------------
# signal types ที่ไม่ควร fire ใน market regime นั้น
REGIME_SIGNAL_BLACKLIST = {
    "bear": {
        "Momentum Continuation 🚀",   # ไม่ต่อเทรนด์ใน bear
        "EMA50 Bounce 🛡️",             # EMA bounce ใน bear มักล้มเหลว
    },
    "sideways": {
        "Momentum Continuation 🚀",   # ไม่มี momentum ใน sideways
    },
    "bull": set(),                     # ใน bull ให้ทุก signal ผ่าน
}
...
_cache_funding:   dict  = {}
_cache_btcd:      dict  = {}
_cache_oi_run:    dict  = {}  # per-run cache, cleared at start of each scan_market()
_cache_macro:     dict  = {}
_cache_24hvol:    dict  = {}
...
def _fetch_realtime_price_coingecko(coin: str) -> float | None:
    """
...
def get_btc_dominance_filter_for_coin(coin: str, btcd_info: dict) -> dict:
    """
    [Upgraded] BTC Dominance filter พร้อม Absolute Threshold:
    - Slope-based (เดิม): alt_unfriendly → -8pts, alt_friendly → +3pts
    - Absolute level (ใหม่): BTC.D > BTCD_ABSOLUTE_BLOCK_THRESHOLD → block entry
      BTC.D > BTCD_ABSOLUTE_CAUTION_THRESHOLD → -5pts additional
    """
    info = {"score_delta": 0, "note": "", "block_entry": False}
    if not btcd_info.get("has_data") or coin in ("BTC", "ETH"):
        return info  # BTC/ETH ไม่ถูก block จาก BTC.D เพราะ BTC.D ขึ้น = BTC แข็ง

    btcd_level = btcd_info.get("btcd_current", 0.0)
    regime = btcd_info.get("regime", "neutral")

    # Absolute hard block
    if BTCD_ABSOLUTE_BLOCK_THRESHOLD and btcd_level >= BTCD_ABSOLUTE_BLOCK_THRESHOLD:
        info["block_entry"] = True
        info["note"] = (f"🚫 BTC.D {btcd_level:.1f}% > {BTCD_ABSOLUTE_BLOCK_THRESHOLD}% "
                        f"— Altcoin season ยังไม่มา, block alt longs")
        info["score_delta"] = -20
        return info

    # Absolute caution zone
    if BTCD_ABSOLUTE_CAUTION_THRESHOLD and btcd_level >= BTCD_ABSOLUTE_CAUTION_THRESHOLD:
        info["score_delta"] -= 5
        info["note"] = f"⚠️ BTC.D สูง ({btcd_level:.1f}%) — ระวัง altcoins"

    # Slope-based (original)
    if regime == "alt_unfriendly":
        info["score_delta"] -= 8
        info["note"] += f" | {btcd_info['btcd_label']}"
...
# ==========================================
_cache_fng: dict  = {}
_cache_ts_fng: float = 0.0

def get_fear_greed_index() -> dict:
    """
    ดึง Crypto Fear & Greed Index จาก api.alternative.me (ฟรี ไม่ต้อง key)
    Returns: {"value": int, "label": str, "score_delta": int, "fng_label": str}
    """
    global _cache_fng, _cache_ts_fng
    now = time.time()
    if _cache_fng and (now - _cache_ts_fng) < CACHE_TTL_SECONDS:
        return _cache_fng

    result = {"value": 50, "label": "Neutral", "score_delta": 0,
              "fng_label": "😐 Fear & Greed: N/A", "has_data": False}

    if not FEAR_GREED_ENABLED:
        _cache_fng, _cache_ts_fng = result, now
        return result

    try:
        resp = api_session.get("https://api.alternative.me/fng/?limit=1", timeout=10)
        if resp.status_code != 200:
            logger.warning(f"⚠️ Fear & Greed: HTTP {resp.status_code}")
            _cache_fng, _cache_ts_fng = result, now
            return result
        ...
        if value <= FEAR_GREED_EXTREME_FEAR:
            result["score_delta"] = FEAR_GREED_SCORE_BONUS
            result["fng_label"] = f"😱 Extreme Fear ({value}) — โอกาสซื้อที่ดี +{FEAR_GREED_SCORE_BONUS}pts"
        elif value <= 40:
            result["score_delta"] = 3
            result["fng_label"] = f"😨 Fear ({value}) — market กลัว +3pts"
        elif value >= FEAR_GREED_EXTREME_GREED:
            result["score_delta"] = -FEAR_GREED_SCORE_PENALTY
            result["fng_label"] = f"🤑 Extreme Greed ({value}) — ระวัง overbought -{FEAR_GREED_SCORE_PENALTY}pts"
        elif value >= 60:
            result["score_delta"] = -3
            result["fng_label"] = f"😎 Greed ({value}) — ระวังเล็กน้อย -3pts"
        else:
            result["fng_label"] = f"😐 Neutral ({value})"

        logger.info(f"✅ Fear & Greed: {value} ({label}) | delta={result['score_delta']:+d}")
    except Exception as e:
...
# ==========================================
# [New] Circuit Breaker (Drawdown Protection)
# ==========================================
def load_circuit_breaker() -> dict:
    """Load circuit breaker state from JSON"""
    if not os.path.exists(CIRCUIT_BREAKER_FILE):
        return {"consecutive_losses": 0, "paused_until": None, "total_sl_hits": 0}
    try:
...
def save_circuit_breaker(state: dict) -> None:
    try:
        ...
        logger.error(f"❌ Save circuit breaker: {e}")

def is_circuit_breaker_active() -> tuple[bool, str]:
    """
    ตรวจว่า Circuit Breaker กำลัง pause อยู่หรือเปล่า
    Returns: (is_paused: bool, reason: str)
    """
    ...
    if paused_until:
        try:
            ...
            if datetime.now(timezone.utc) < until_dt:
                remaining_h = (until_dt - datetime.now(timezone.utc)).total_seconds() / 3600
                return True, (f"⛔ Circuit Breaker Active: {state['consecutive_losses']} SL hits ติดกัน "
                              f"— pause อีก {remaining_h:.1f}h จนถึง {until_dt.strftime('%H:%M UTC')}")
        except Exception:
            pass
    return False, ""

def record_sl_hit(coin: str) -> None:
    """เรียกเมื่อ position โดน SL — อัปเดต consecutive_losses"""
    if not CIRCUIT_BREAKER_ENABLED:
        return
    state = load_circuit_breaker()
    state["consecutive_losses"] = state.get("consecutive_losses", 0) + 1
    state["total_sl_hits"]      = state.get("total_sl_hits", 0) + 1

    if state["consecutive_losses"] >= CIRCUIT_BREAKER_MAX_LOSSES:
        pause_until = datetime.now(timezone.utc)
        pause_until = pause_until.replace(
            hour=0, minute=0, second=0, microsecond=0
        )
        from datetime import timedelta
        pause_until += timedelta(hours=CIRCUIT_BREAKER_PAUSE_HOURS)
        state["paused_until"] = pause_until.isoformat()
        logger.warning(
            f"🔴 Circuit Breaker TRIGGERED: {state['consecutive_losses']} consecutive SL hits "
            f"({coin} was last) — pausing until {pause_until.strftime('%Y-%m-%d %H:%M UTC')}"
        )
        send_telegram(
            f"🔴 <b>Circuit Breaker Activated!</b>\n"
            f"โดน SL {state['consecutive_losses']} ครั้งติดกัน ({coin} ล่าสุด)\n"
            f"ระงับการเปิด position ใหม่ {CIRCUIT_BREAKER_PAUSE_HOURS}h\n"
            f"จนถึง: {pause_until.strftime('%Y-%m-%d %H:%M UTC')}"
        )

    save_circuit_breaker(state)

def reset_consecutive_losses() -> None:
    """เรียกเมื่อ position ถึง TP1 — reset consecutive losses"""
    if not CIRCUIT_BREAKER_ENABLED:
        return
    state = load_circuit_breaker()
    if state.get("consecutive_losses", 0) > 0:
        ...
        save_circuit_breaker(state)
        logger.info("✅ Circuit Breaker reset (TP1 hit)")


# ==========================================
# [New] Win Rate Tracking
# ==========================================
def record_trade_outcome(coin: str, signal_type: str, outcome: str,
                         pnl_pct: float, entry_price: float,
                         exit_price: float, bars_held: int = 0) -> None:
    """
    บันทึก trade outcome ใน trade_log.jsonl
    outcome: "tp1" | "tp1_5" | "tp2" | "sl" | "time_stop" | "manual"
    """
    record = {
    ...
    try:
        ...
        logger.error(f"❌ record_trade_outcome: {e}")

def compute_win_rate_summary() -> dict:
    """
    คำนวณ win rate จาก trade_log.jsonl แยกตาม signal_type
    Returns dict สำหรับใส่ใน Telegram summary
    """
    ...
    for t in trades:
        ...
        is_win = out in ("tp1", "tp1_5", "tp2") or pnl > 0
        stats[sig]["wins" if is_win else "losses"] += 1
        stats[sig]["total_pnl"] += pnl
...
def format_win_rate_report() -> str:
    """สร้าง Telegram message สำหรับ Win Rate Report"""
    stats = compute_win_rate_summary()
    if not stats:
        return "📊 Win Rate: ยังไม่มีข้อมูล trade"

    lines = ["📊 <b>Win Rate Report</b>"]
    total_trades = sum(s["trades"] for s in stats.values())
    total_wins   = sum(s["wins"]   for s in stats.values())
    overall_wr   = total_wins / total_trades * 100 if total_trades else 0

    lines.append(f"Overall: {overall_wr:.1f}% ({total_wins}/{total_trades} trades)\n")

    for sig, s in sorted(stats.items(), key=lambda x: -x[1]["win_rate"]):
        icon = "🟢" if s["win_rate"] >= 55 else "🟡" if s["win_rate"] >= 45 else "🔴"
        lines.append(
            f"{icon} {sig[:30]}\n"
            f"   WR: {s['win_rate']}% | {s['trades']} trades | avg PnL: {s['avg_pnl']:+.2f}%"
        )
    return "\n".join(lines)
...
def get_mtf_rsi_alignment(df_4h: pd.DataFrame, df_1d: pd.DataFrame) -> dict:
    """
    [Advanced] Multi-Timeframe RSI Alignment พร้อม:
    - 3 Timeframes: 4H + 1D + 1W (ดึงจาก 1D resample)
    - RSI Slope (momentum vector): ทิศทาง RSI กำลังเปลี่ยนไปทางไหน ไม่ใช่แค่ระดับ
    - Confluence Grading: คะแนนแยกตาม oversold depth + slope alignment
    - Divergence flag: ถ้า 4H/1D/1W RSI ขัดแย้งกัน (e.g. 4H oversold แต่ 1W overbought)
    """
    result = {
        "aligned_oversold": False, "aligned_overbought": False,
        "rsi_4h": None, "rsi_1d": None, "rsi_1w": None,
    ...
    # --- Confluence Score (weighted by timeframe + depth + slope) ---
    score = 0
    ...
    def _rsi_oversold_score(rsi: float, slope: float) -> int:
        ...
        if slope > 0:   s += 1   # RSI กำลังดีดขึ้น = extra confirmation
        return s
    ...
    # --- Conflict detection: 4H oversold but 1W overbought (or vice versa) ---
    ...
    # --- Label ---
    rsi_str = f"4H:{rsi_4h:.1f}/1D:{rsi_1d:.1f}/1W:{rsi_1w:.1f}"
    slope_str = f"slope4H:{slope_4h:+.1f}"
    if total_os >= 9:
        result.update({"aligned_oversold": True,
                        "mtf_label": f"💎 MTF Oversold แข็งแกร่งมาก ({rsi_str}){conflict_note}"})
    elif total_os >= 5:
        result.update({"aligned_oversold": True,
                        "mtf_label": f"🔵 MTF Oversold ({rsi_str}) | {slope_str}{conflict_note}"})
    elif total_os >= 2:
        result["mtf_label"] = f"🟡 MTF RSI อ่อนแรง ({rsi_str}){conflict_note}"
    elif total_ob >= 6:
...
def calculate_signal_score(rsi, bounce_info, candle_info, vol_confirmed, in_fibo_zone, in_ob_zone,
                            ...
                            div_info=None, bounce_score=None, signal_type: str = "",
                            fng_info=None, **kwargs) -> tuple[int, str]:
    """
    ...
    # ─────────────────────────────────────────────────────────────
    if market_regime == "bull" and 40 <= rsi <= 65:
        # RSI อยู่ใน healthy pullback zone ของ bull regime
        rsi_base = 14 if rsi <= 50 else 10 if rsi <= 55 else 6
    else:
    ...
    """
    [Advanced] Weighted Signal Scoring ที่ปรับตาม Market Regime

    เดิม: linear sum ทุก factor เท่ากัน
    ใหม่:
    1. Market Regime detection (Bull/Bear/Sideways) — ปรับ weight แต่ละ factor
    2. Factor Interaction Bonus: หลาย factor confluence พร้อมกัน -> bonus สะสม
    3. Advanced Divergence Score (จาก div_info)
    4. RSI Bounce Score (จาก bounce_info["bounce_score"] ที่แม่นขึ้น)
    5. Penalty layer: ลงโทษ signal ที่ขัดแย้งกับ MTF / เทรนด์หลัก
    """
    market_regime = "neutral"
    if trend_info:
        ts = trend_info.get("trend_strength", "sideways")
    ...
    if market_regime == "bull":
        # ใน uptrend: dip ที่ bounce จาก support มีน้ำหนักสูง
        w_rsi=1.0; w_bounce=1.2; w_candle=1.1; w_vol=1.2; w_level=1.1; w_div=1.0; w_mtf=1.1
    elif market_regime == "bear":
        # ใน downtrend: สัญญาณซื้อต้องแข็งแกร่งกว่ามากจึงจะเชื่อถือได้
        w_rsi=0.8; w_bounce=0.8; w_candle=0.9; w_vol=1.3; w_level=1.2; w_div=1.3; w_mtf=1.2
    else:  # sideways
        # ใน sideways: volume + key level สำคัญที่สุด, streak ไม่น่าเชื่อถือ
        w_rsi=0.9; w_bounce=0.9; w_candle=1.0; w_vol=1.4; w_level=1.3; w_div=1.1; w_mtf=1.0

    score = 0.0

    # --- 1. RSI Oversold ---
    # RSI scoring: ปรับให้ทำงานกับทั้ง "dip" signals (RSI ต่ำ) และ
    # "continuation/pullback" signals (RSI 40-65 ใน uptrend)
    # สัญญาณประเภท Momentum Continuation / Pullback in Uptrend / EMA50 Bounce
    # จะมี RSI 40-65 ซึ่งไม่ oversold — ไม่ควรได้ 0 คะแนน
    is_continuation_signal = any(kw in str(locals().get("signal_type_hint", "")) for kw in
                                  ["Momentum", "Pullback", "EMA50"])
    if market_regime == "bull" and 40 <= rsi <= 65:
        # ใน bull regime + RSI อยู่ใน healthy pullback zone = ดี ไม่ใช่ neutral
        rsi_base = 12 if rsi <= 55 else 6
    else:
        rsi_base = (25 if rsi <= RSI_OVERSOLD else 15 if rsi <= 40 else 8 if rsi <= 50 else 0)
    score += rsi_base * w_rsi

    # --- 2. RSI Bounce (advanced score from bounce_info) ---
    b_score = bounce_info.get("bounce_score", 0) if bounce_info else 0
    b_quality = bounce_info.get("quality", "none") if bounce_info else "none"
    bounce_pts = {"strong": 20, "moderate": 12, "weak": 5, "none": 0}.get(b_quality, 0)
    # Use bounce_score (0-12) as additional detail within quality band
    bounce_pts = max(bounce_pts, min(b_score * 2, 22))
    score += bounce_pts * w_bounce

    # --- 3. Candle Patterns (capped, context-aware) ---
    candle_raw = min(candle_info.get("reversal_strength", 0) * 6, 18) if candle_info else 0
    # Extra bonus if pattern is at key level (new advanced field)
    if candle_info and candle_info.get("at_key_level"):
        candle_raw = min(candle_raw + 4, 22)
    score += candle_raw * w_candle
    ...
    if in_ob_zone:   level_score += 7
    if in_fvg_zone:  level_score += 5
    # Confluence bonus: 2+ levels at once
    levels_hit = sum([in_fibo_zone, in_ob_zone, in_fvg_zone])
    if levels_hit >= 2: level_score += 5
    if levels_hit == 3: level_score += 5
    score += level_score * w_level

    # --- 5. Divergence (advanced 4-type with strength) ---
    if div_info:
        div_pts = 0
        if div_info.get("regular_bullish"):
            div_pts += 8 + div_info.get("regular_bullish_strength", 0) * 3
        if div_info.get("hidden_bullish"):
            div_pts += 5 + div_info.get("hidden_bullish_strength", 0) * 2
        score += min(div_pts, 20) * w_div
    elif is_divergence:
        score += 10 * w_div

    # --- 6. Volume ---
    if vol_confirmed: score += 10 * w_vol

    # --- 7. MTF Alignment (advanced 3-TF confluence) ---
    mtf_raw = min(mtf_info.get("confluence_score", 0) * 2, 12) if mtf_info else 0
    ...
    if weekly_ctx:
        if weekly_ctx.get("weekly_bullish_div"): score += 6
        elif weekly_ctx.get("rsi_weekly") and weekly_ctx["rsi_weekly"] <= 35: score += 4
        # Wyckoff accumulation bonus
        if weekly_ctx.get("wyckoff_phase") == "accumulation": score += 5
        # Near a Fibo extension (TP target alignment)
        if weekly_ctx.get("nearest_fibo_pct") is not None and weekly_ctx["nearest_fibo_pct"] <= 1.0:
            score += 3
    ...
    if trend_info:
        er = trend_info.get("efficiency_ratio", 0)
        if er >= 0.6 and market_regime == "bull":  score += 4
        if market_regime == "bear": score -= 5  # penalize all longs in bear regime
    ...
    if funding_info and funding_info.get("has_data"):
        if funding_info.get("warn_long"):   score -= 5
        fr = funding_info.get("funding_rate", 0)
        if fr is not None and fr < 0:       score += 3  # negative funding = long-friendly

    # --- 12. OI ---
    if oi_info and oi_info.get("has_data") and oi_info.get("weak_conviction"):
        score -= 8

    # --- 13. BTC Dominance (slope + absolute) ---
    if btcd_filter and btcd_filter.get("score_delta"):
        score += btcd_filter["score_delta"]

    # --- 14. Fear & Greed Index ---
    # ส่งเป็น parameter fng_info (optional) เพื่อไม่ต้อง fetch ใหม่ทุก call
    fng_info = kwargs.get("fng_info")
    if fng_info and fng_info.get("has_data"):
        score += fng_info.get("score_delta", 0)

    # --- 14. Trend Structure Score (สำหรับ Pullback/Momentum signals) ---
    # สัญญาณใหม่ (Pullback in Uptrend, Momentum Continuation, EMA50 Bounce)
    # ต้องมีแหล่งคะแนนเพิ่มเติมจาก structure/momentum แทนที่จะพึ่งแค่ oversold RSI
    if trend_info:
        hh = trend_info.get("higher_high", False)
        hl = trend_info.get("higher_low", False)
        er = trend_info.get("efficiency_ratio", 0.0)
        ...
        # Higher High + Higher Low structure = uptrend confirmed
        if hh and hl and market_regime == "bull":
            score += 10
        elif hh or hl:
            score += 4

        # Efficiency Ratio: ยิ่งสูง = ราคาเดินทางตรง = momentum แข็ง
        if er >= 0.6:   score += 8
        elif er >= 0.4: score += 4

        # Trend consistency: % ของแท่งที่เดินทางไปทิศเดียวกับ trend
        if consistency >= 0.65: score += 5
        elif consistency >= 0.55: score += 2

    # --- Confluence Interaction Bonus ---
    # เมื่อ factor หลายตัว fire พร้อมกัน = สัญญาณแข็งกว่าผลรวมแต่ละตัว
    confluence_factors = sum([
        rsi <= RSI_OVERSOLD,
        b_quality in ("strong", "moderate"),
        in_fibo_zone or in_ob_zone or in_fvg_zone,
        ...
        vol_confirmed,
        mtf_info.get("aligned_oversold", False) if mtf_info else False,
        # เพิ่ม factor สำหรับ trending signals
        bool(trend_info and trend_info.get("higher_high") and trend_info.get("higher_low")),
        bool(trend_info and trend_info.get("efficiency_ratio", 0) >= 0.5),
    ])
    if confluence_factors >= 5: score += 10
    elif confluence_factors >= 4: score += 5
    elif confluence_factors >= 3: score += 2  # เพิ่ม tier ล่างสุด

    score = max(0, min(100, int(round(score))))
    grade = "🔥 A+" if score >= 75 else "✅ A" if score >= 60 else "🟡 B" if score >= MINIMUM_SIGNAL_SCORE else "⬜ C"
    regime_tag = {"bull": "📈Bull", "bear": "📉Bear", "sideways": "↔️Side"}.get(market_regime, "")
    return score, f"{grade} | Score: {score}/100 | {regime_tag}"
...
def scan_market(positions: dict):
    ...
    logger.info("Phase 8: Fetch Fear & Greed Index...")
    fng_info = get_fear_greed_index()

    # [Circuit Breaker] ตรวจก่อนเริ่ม scan
    cb_paused, cb_reason = is_circuit_breaker_active()
    if cb_paused:
    ...
    for coin in COINS:
        ...
        if price > ema200:
            ...
            # RSI ขยายจาก 55 → 65 เพื่อรองรับเหรียญที่กำลัง trending
            # ─────────────────────────────────────────────────────────────
            if (in_fibo or in_ob or in_fvg) and price > (ema50 * 0.97) and rsi <= 65:
            ...
            # ─────────────────────────────────────────────────────────────
            # Signal C: Pullback in Uptrend (ใหม่)
            # เหรียญ uptrend แข็ง (HH+HL) แล้ว RSI ดึงกลับมาพักในโซน 40-55
            # แล้วดีดกลับ — เหมาะกับ "ช้อนซื้อใน pullback ของ trend"
            # ─────────────────────────────────────────────────────────────
            if (not signal_type
                    and ts in ("strong_up", "moderate_up")
                    and hh and hl                          # structure ยืนยัน uptrend
                    and 38 <= rsi <= 58                    # อยู่ในโซน pullback (ไม่ oversold มาก, ไม่ร้อนเกิน)
            ...
            if coin in positions:
                ...
                # ราคาหลุด EMA200 = สัญญาณร้ายแรงมาก → บวกคะแนนเพิ่ม
                if price < ema200:
                    exit_score_b = min(100, exit_score_b + 25)
                    exit_reasons_b.insert(0, f"🔴 ราคาหลุด EMA200 ({format_price(ema200)}) — สถานะ Long เสี่ยงสูง")
                    exit_label_b = f"🚨 <b>ราคาหลุด EMA200 — พิจารณาปิดสถานะ Long</b> | Exit Score: {exit_score_b}/100"
                ...
                if exit_score_b >= eff_threshold_b or price < ema200:
                    ...
                    exit_watch_signals.append({
                        ...
                        "below_ema200_position": True,  # flag พิเศษสำหรับ Telegram
                    })
        ...
        if signal_type and coin not in positions:
            ...
            # [Circuit Breaker] ไม่เปิด position ใหม่ถ้า paused
            if cb_paused:
                logger.info(f"⛔ {coin}: circuit breaker active — {cb_reason[:60]}")
                continue

            # [Regime-Aware Signal Blacklist] block signal types ที่ไม่เหมาะกับ regime
            regime_key = "bear" if "Bear" in market_regime else ("sideways" if "Sideways" in market_regime else "bull")
            blacklisted = REGIME_SIGNAL_BLACKLIST.get(regime_key, set())
            if signal_type in blacklisted:
                logger.info(f"⛔ {coin}: signal '{signal_type}' blacklisted ใน {regime_key} regime")
                continue

            # [BTC.D Absolute Block] block alt longs ถ้า BTC.D สูงเกิน threshold
            if btcd_filter.get("block_entry") and coin not in ("BTC", "ETH"):
                logger.info(f"⛔ {coin}: BTC.D absolute block — {btcd_filter.get('note','')[:60]}")
                continue
            ...
            sig_score, score_label = calculate_signal_score(
                ...
                div_info=div_info, signal_type=signal_type, fng_info=fng_info,
            )
            ...
            if sig_score >= MINIMUM_SIGNAL_SCORE:
                ...
                # [Adaptive Position Sizing] ปรับ size ตาม signal score
                if ADAPTIVE_SIZING_ENABLED:
                    size_factor = min(sig_score / ADAPTIVE_SCORE_REFERENCE, 1.5)
                    adaptive_risk = max(ADAPTIVE_MIN_RISK_PCT,
                                       min(RISK_PER_TRADE_PCT * size_factor, ADAPTIVE_MAX_RISK_PCT))
                    pos_size = get_correlation_adjusted_position(
                        PORTFOLIO_USDT, adaptive_risk, sl_dist, corr_btc, active_signal_count)
                    adaptive_note = (f"📐 Size: {adaptive_risk:.1f}% risk "
                                     f"(score {sig_score} × factor {size_factor:.2f})")
                else:
                    adaptive_risk = RISK_PER_TRADE_PCT
                    pos_size = get_correlation_adjusted_position(
                        PORTFOLIO_USDT, RISK_PER_TRADE_PCT, sl_dist, corr_btc, active_signal_count)
                    adaptive_note = ""
                ...
                buy_signals.append({
                    ...
                    "fng_label": fng_info.get("fng_label", "") if fng_info else "",
                    "fee_note": (f"💳 Fee adjusted: TP1 net ≈ ${format_price(tp1_val * (1 - TRADING_FEE_PCT))} "
                                 f"| SL net ≈ ${format_price(sl_val * (1 + TRADING_FEE_PCT))}"),
                    "partial_tp_note": (
    ...
    if stale_coins:
        stale_line = (
            ...
            f"— ข้อมูลอาจไม่ตรงกับตลาดจริง"
        )

    cb_line = f"\n{cb_reason}" if cb_paused else ""
    fng_line = f"\n{fng_info.get('fng_label','')}" if fng_info and fng_info.get("has_data") else ""
    btcd_abs_line = ""
    btcd_level = btcd_info.get("btcd_current", 0.0)
    if btcd_level >= BTCD_ABSOLUTE_CAUTION_THRESHOLD:
        btcd_abs_line = f" | BTC.D: {btcd_level:.1f}%{'🚫' if btcd_level >= BTCD_ABSOLUTE_BLOCK_THRESHOLD else '⚠️'}"

    summary_msg = (
        f"📊 <b>[Market Summary – v10]</b>\n"
        f"ดัชนีหลัก (BTC): <b>{market_regime}</b>\n"
        f"🌐 {btcd_info.get('btcd_label', '⚪ N/A BTC.D')}{btcd_abs_line}\n"
...
def build_messages(buy_list, exit_watch_list, position_updates, market_summary, supply_zone_alerts=None) -> list:
    ...
    if buy_list:
        ...
        for opt in buy_list:
            ...
            fng_line       = f"\n{opt['fng_label']}" if opt.get("fng_label") else ""
            adaptive_line  = f"\n{opt['adaptive_note']}" if opt.get("adaptive_note") else ""
            fee_line       = f"\n{opt['fee_note']}" if opt.get("fee_note") else ""
            msg = (
    ...
    if exit_watch_list:
        ...
        for opt in sorted(exit_watch_list, key=lambda x: -x["exit_score"]):
            ...
            rsi_header = ""
            if opt.get("below_ema200_position"):
            ...
            msg = (
                f"\n\n🪙 <b>{opt['coin']}</b> | ราคา: ${opt['price']} | RSI: {opt['rsi']}{ext_line}\n"
                f"{rsi_header}"
                f"{opt['exit_label']}\n"
...
if __name__ == "__main__":
    ...
    if "--win-rate" in sys.argv:
        print(format_win_rate_report())
        raise SystemExit(0)

    try:
        ip = api_session.get("https://ifconfig.me", timeout=5).text.strip()
        logger.info(f"🌐 Network IP: {ip}")
    except Exception:
        pass

    try:
        positions = load_positions()
        buy_list, exit_watch_list, position_updates, market_summary, supply_zone_alerts = scan_market(positions)
        send_telegram_messages(build_messages(buy_list, exit_watch_list, position_updates, market_summary, supply_zone_alerts))
        save_positions(positions)
        logger.info("ระบบทำงานสมบูรณ์!")

        # Weekly Win Rate Report (ทุก Monday 00:00-04:00 UTC)
        now_utc = datetime.now(timezone.utc)
        if now_utc.weekday() == 0 and now_utc.hour < 4:
            report = format_win_rate_report()
            if report:
        ...
        # [Error Recovery] แจ้ง Telegram ทันทีถ้า script crash
        err_msg = (f"🆘 <b>Screener Error!</b>\n"
                   f"Script crash: <code>{type(e).__name__}: {str(e)[:200]}</code>\n"
                   f"Time: {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}\n"
                   f"กรุณาตรวจสอบ GitHub Actions logs")
